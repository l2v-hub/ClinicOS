...
try {
  ...
  res = await af(`${base}/${jobId}/files`, {
    ...
    body: form([
      {
        name: 'pagina2.png',
        buf: Buffer.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a, 1, 2]),
        type: 'image/png',
      },
    ]),
  });
  ...
  body = await res.json();
  assert.equal(
    body.documents.find((d: { id: string; logicalDoc?: string }) => d.id === someDoc)?.logicalDoc,
    'Documento 1',
    'logicalDoc persisted',
  );
  ...
  assert.equal(
    body.status,
    'review_ready',
    `worker -> review_ready (got ${body.status} / ${body.error ?? ''})`,
  );
  ...
  assert.equal(
    result.resultData.anagrafica.nome.status,
    'missing',
    'mock -> empty -> missing, no invention',
  );
