...
const ORIGINAL = [
  ...
  '',
  '## Decorso ospedaliero',
  'Giorno 1: monitoraggio. Giorno 5: dimissione.',
  '',
  'Pagina 2 di 3',
  '',
  HEADER,
...
writeFileSync(
  join(OUT, 'header-footer-detected.txt'),
  [
...
const between = r.cleanedText.slice(
  r.cleanedText.indexOf('Inviata in PS'),
  r.cleanedText.indexOf('Proseguiva'),
);
writeFileSync(
  join(OUT, 'cross-page-text-without-header.txt'),
  [
...
console.log('REQ-037 evidence written:', {
  removedHeaderBlocks: r.removedHeaderBlocks,
  removedFooterLines: r.removedFooterLines,
  headerOccurrences,
  pages: r.detectedPageNumbers,
});
