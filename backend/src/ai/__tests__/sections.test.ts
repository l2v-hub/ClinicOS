...
test('schema rejects an unknown sectionKey and unknown tag', () => {
  assert.equal(
    validateSectionsSchema({ sections: [{ sectionKey: 'NOPE', rawText: 'x' }] }).valid,
    false,
  );
  assert.equal(
    validateSectionsSchema({
      sections: [
        {
          ...
          rawText: 'x',
          annotations: [{ tag: 'BOLD', text: 'x', startOffset: 0, endOffset: 1 }],
        },
      ],
    }).valid,
    false,
    ...
    sections: [
      {
        ...
        rawText,
        annotations: [
          ann(rawText, 'Anamnesi patologica remota', 'SUBSECTION_TITLE'),
          ann(rawText, 'Anamnesi familiare', 'SUBSECTION_TITLE'),
        ],
      },
    ],
  });
...
test('hospital course: multiple dates annotated, sequence preserved', () => {
  const rawText =
    'Il 12/03/2025 ingresso in reparto. Il 15/03/2025 stabilizzazione. Dimesso il 18/03/2025.';
  const r = pp({
    sections: [
      {
        ...
        rawText,
        annotations: [
          ann(rawText, '12/03/2025', 'DATE'),
          ann(rawText, '15/03/2025', 'DATE'),
          ann(rawText, '18/03/2025', 'DATE'),
        ],
      },
    ],
  });
        ...
        medications: [
          {
            ...
            schedule: '1 cp',
            frequency: 'al giorno',
            exactText: 'Ramipril 5 mg 1 cp al giorno.',
          },
        ],
      },
    ],
...
test('explicit allergy -> present', () => {
  const r = pp({
    sections: [{ sectionKey: 'ALLERGIES', rawText: 'Allergia a Penicillina.' }],
    allergies: { status: 'present', rawText: 'Allergia a Penicillina.' },
  });
...
test('explicitly declared absence -> explicitly_absent', () => {
  const r = pp({
    sections: [{ sectionKey: 'ALLERGIES', rawText: 'Allergie: nessuna nota.' }],
    allergies: { status: 'explicitly_absent' },
  });
...
test('illegible allergy -> unclear, text not interpreted', () => {
  ...
  const r = pp({
    sections: [{ sectionKey: 'ALLERGIES', rawText: raw }],
    allergies: { status: 'unclear', rawText: raw, warnings: ['ALLERGY_TEXT_PARTIALLY_UNREADABLE'] },
  });
...
test('conflicting allergy info blocks confirmation', () => {
  const r = pp({
    sections: [
      { sectionKey: 'ALLERGIES', rawText: 'Allergia a Penicillina. Nessuna allergia nota.' },
    ],
    allergies: { status: 'conflicting' },
  });
    ...
    sections: [
      {
        ...
        rawText,
        annotations: [{ tag: 'DATE', text: '09/09/2025', startOffset: 0, endOffset: 3 }],
      },
    ],
  });
    ...
    sections: [
      {
        sectionKey: 'HOSPITAL_COURSE',
        rawText,
        annotations: [{ tag: 'DATE', text: '01/01/2099', startOffset: 0, endOffset: 10 }],
      },
    ],
  });
    ...
    sections: [
      {
        ...
        rawText,
        annotations: [
          { tag: 'SECTION_TITLE', text: '<b>Diagnosi</b>', startOffset: 0, endOffset: 15 },
        ],
      },
    ],
  });
    ...
    sections: [
      {
        sectionKey: 'ANAMNESIS',
        rawText: 'Parte A.',
        annotations: [ann('Parte A.', 'Parte A.', 'SUBSECTION_TITLE')],
      },
      {
        sectionKey: 'ANAMNESIS',
        rawText: 'Parte B.',
        annotations: [ann('Parte B.', 'Parte B.', 'SUBSECTION_TITLE')],
      },
    ],
  });
