...
test('produces a flat narrative draft — strings, never arrays (no diagnoses[])', () => {
  const d = buildNarrativeDraft(
    result({
      sections: [
        { sectionKey: 'DISCHARGE_DIAGNOSIS', rawText: 'Scompenso cardiaco.\n\nIpertensione.' },
      ],
    }),
  );
...
test('valid draft passes the flat contract schema', () => {
  const d = buildNarrativeDraft(
    result({ sections: [{ sectionKey: 'DISCHARGE_DIAGNOSIS', rawText: 'BPCO.' }] }),
  );
...
test('diagnosis multi-paragraph stays one block', () => {
  ...
  const d = buildNarrativeDraft(
    result({ sections: [{ sectionKey: 'DISCHARGE_DIAGNOSIS', rawText: raw }] }),
  );
      ...
      sections: [
        {
          ...
          rawText: raw,
          annotations: [
            ann(raw, 'Anamnesi patologica remota', 'SUBSECTION_TITLE'),
            ann(raw, 'Anamnesi familiare', 'SUBSECTION_TITLE'),
          ],
        },
      ],
    }),
      ...
      sections: [
        {
          ...
          rawText: raw,
          annotations: [ann(raw, '12/03/2025', 'DATE'), ann(raw, '15/03/2025', 'DATE')],
        },
      ],
    }),
  );
      ...
      sections: [
        {
          ...
          rawText: hosp,
          annotations: [ann(hosp, 'Furosemide', 'MEDICATION_NAME')],
        },
        {
          sectionKey: 'DISCHARGE_HOME_THERAPY',
          rawText: home,
          annotations: [ann(home, 'Ramipril', 'MEDICATION_NAME')],
        },
      ],
    }),
  );
...
test('partially illegible therapy line kept integrally', () => {
  ...
  const d = buildNarrativeDraft(
    result({ sections: [{ sectionKey: 'DISCHARGE_HOME_THERAPY', rawText: raw }] }),
  );
...
test('advice and follow-up kept integrally', () => {
  ...
  const d = buildNarrativeDraft(
    result({ sections: [{ sectionKey: 'ADVICE_AND_FOLLOW_UP', rawText: raw }] }),
  );
  assert.equal(d.adviceAndFollowUpText, raw);
});

test('allergies — present/absent/not_documented/unclear faithful', () => {
  const present = buildNarrativeDraft(
    result({
      allergies: { status: 'present', rawText: 'Penicillina.' },
      sections: [
        {
          sectionKey: 'ALLERGIES',
          rawText: 'Penicillina.',
          annotations: [ann('Penicillina.', 'Penicillina', 'ALLERGY_CRITICAL')],
        },
      ],
    }),
  );
  ...
  assert.equal(
    buildNarrativeDraft(result({ allergies: { status: 'explicitly_absent' } })).allergyStatus,
    'explicitly_absent',
  );
  assert.equal(buildNarrativeDraft(result({})).allergyStatus, 'not_documented');

  const unclear = buildNarrativeDraft(
    result({ allergies: { status: 'unclear', rawText: 'Allergia a [ILLEGGIBILE]' } }),
  );
...
test('unmapped content not lost; missing sections listed', () => {
  const d = buildNarrativeDraft(
    result({ sections: [{ sectionKey: 'UNMAPPED_CONTENT', rawText: 'Timbro reparto.' }] }),
  );
