...
test('016 F3: field resolves incl 1-hop relation; unknown field/relation denied', () => {
  ...
  assert.ok(resolveField('roomAssignment', 'bed.label'));
  assert.ok(resolveField('roomAssignment', 'patient.lastName'));
  assert.equal(resolveField('roomAssignment', 'bed.secret'), null);
  assert.equal(resolveField('roomAssignment', 'ssn'), null);
  assert.equal(resolveField('roomAssignment', 'notARelation.x'), null);
});

test('016 F3: roomAssignment has no direct room relation (2-hop path denied)', () => {
  assert.equal(resolveField('roomAssignment', 'room.numero'), null);
  assert.equal(resolveField('roomAssignment', 'bed.room.numero'), null); // >1 hop
});
