...
test('016 F3: count aggregate returns a number (empty DB → 0)', async () => {
  const plan = validateQueryPlan({ steps: [{ id: 's1', from: 'roomAssignment', filter: [{ field: 'endDate', op: 'isNull' }], aggregate: { op: 'count' } }] })!;
...
test('016 F3: relate 1-hop does not crash (patient + bed on roomAssignment)', async () => {
  const plan = validateQueryPlan({ steps: [{ id: 's1', from: 'roomAssignment', filter: [{ field: 'endDate', op: 'isNull' }], relate: ['patient', 'bed'], select: ['patient.lastName', 'bed.label'], limit: 20 }] })!;
  const out = await runQueryPlan(plan, CTX, ON);
  assert.ok(Array.isArray(out.rows));
});
...
test('016 F3: runIf skips step2 when step1 empty', async () => {
  const plan = validateQueryPlan({
    steps: [
      { id: 's1', from: 'room', filter: [{ field: 'numero', op: 'eq', value: '__no_such_room__' }], select: ['numero'] },
...
test('016 F3: tenant isolation rejects a foreign tenant', async () => {
  const plan = validateQueryPlan({ steps: [{ id: 's1', from: 'room', aggregate: { op: 'count' } }] })!;
