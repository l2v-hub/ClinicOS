...
test('016 F1: piano LLM valido (solo read tools) è accettato', async () => {
  const runtime = async () => ({
    plan: {
      ...
      tools: [{ tool: 'get_patient_allergies', args: { patientId: 'p1' } }],
      requiresCrossPatientAccess: false,
    },
    confidence: 0.9,
  });
  const r = await planQueryLLM(
    'allergie di Rossi',
    { currentPatientId: 'p1' },
    { callPlanRuntime: runtime },
  );
...
test('016 F1: piano LLM con tool FUORI allowlist → fallback deterministico', async () => {
  const runtime = async () => ({
    plan: {
      intent: 'unknown',
      scope: 'current_patient',
      tools: [{ tool: 'delete_patient', args: { patientId: 'p1' } }],
      requiresCrossPatientAccess: false,
    },
    confidence: 0.9,
  });
...
test('016 F1: scope=cross dichiarato dall LLM su tool patient-scoped è IGNORATO (no falso rifiuto)', async () => {
  ...
  const runtime = async () => ({
    plan: {
      ...
      scope: 'cross_patient',
      tools: [
        { tool: 'search_patients', args: { query: 'Folli' } },
        { tool: 'get_patient_allergies', args: { patientId: 'x' } },
      ],
      requiresCrossPatientAccess: true,
    },
    confidence: 0.9,
  });
  const r = await planQueryLLM(
...
test('016 F1: requiresCrossPatientAccess è RICALCOLATO server-side, non fidato dall LLM', async () => {
  ...
  const runtime = async () => ({
    plan: {
      intent: 'correlate',
      scope: 'cross_patient',
      tools: [{ tool: 'correlate_structured_data', args: {} }],
      requiresCrossPatientAccess: false,
    },
    confidence: 0.9,
  });
  const r = await planQueryLLM(
    'quali pazienti hanno allergia alla penicillina',
    {},
    { callPlanRuntime: runtime },
  );
...
test('016 F1: injectPatientId riempie patientId mancante nei tool patient-scoped', () => {
  const plan = {
    intent: 'allergies' as const,
    scope: 'current_patient' as const,
    tools: [{ tool: 'get_patient_allergies', args: {} }],
    requiresCrossPatientAccess: false,
  };
...
test('016 F1: injectPatientId SOVRASCRIVE il patientId proposto dall LLM (paziente autoritativo)', () => {
  ...
  const plan = {
    intent: 'therapies' as const,
    scope: 'current_patient' as const,
    tools: [{ tool: 'get_patient_therapies', args: { patientId: 'pX' } }],
    requiresCrossPatientAccess: false,
  };
  const out = injectPatientId(plan, 'p42');
  assert.equal(out.tools[0].args.patientId, 'p42');
...
test('016 F1: injectPatientId sovrascrive un patientId PLACEHOLDER dell LLM', () => {
  ...
  const plan = {
    intent: 'allergies' as const,
    scope: 'current_patient' as const,
    tools: [{ tool: 'get_patient_allergies', args: { patientId: '<resolved_patientId>' } }],
    requiresCrossPatientAccess: false,
  };
  const out = injectPatientId(plan, 'p42');
  assert.equal(out.tools[0].args.patientId, 'p42');
...
test('016 F1: injectPatientId NON tocca i tool non patient-scoped (search_patients)', () => {
  const plan = {
    intent: 'patient_search' as const,
    scope: 'current_patient' as const,
    tools: [{ tool: 'search_patients', args: { query: 'Rossi' } }],
    requiresCrossPatientAccess: false,
  };
  const out = injectPatientId(plan, 'p42');
