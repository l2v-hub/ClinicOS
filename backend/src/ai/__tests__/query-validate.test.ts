    ...
    steps: [
      {
        ...
        from: 'vitalSign',
        filter: [
          { field: 'patient', op: 'eq', value: 'Folli' },
          { field: 'etichetta', op: 'eq', value: 'PA' },
        ],
      },
    ],
  });
...
test('016 F3: bad operator rejected', () => {
  assert.equal(
    validateQueryPlan({
      steps: [
        { id: 's1', from: 'room', filter: [{ field: 'numero', op: 'DROP' as never, value: 1 }] },
      ],
    }),
    null,
  );
...
test('016 F3: bindFrom referencing prior step accepted; unknown step rejected', () => {
  assert.ok(
    validateQueryPlan({
      steps: [
        { id: 's1', from: 'roomAssignment', select: ['patientId'] },
        { id: 's2', from: 'patient', bindFrom: { step: 's1', field: 'patientId', into: 'id' } },
      ],
    }),
  );
  assert.equal(
    validateQueryPlan({
      steps: [
        { id: 's2', from: 'patient', bindFrom: { step: 'sX', field: 'patientId', into: 'id' } },
      ],
    }),
    null,
  );
