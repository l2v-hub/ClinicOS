...
import { validateFile, sanitizeFilename, sha256Hex, ACCEPTED_EXTENSIONS } from '../upload/validation.js';
...
const WEBP = Buffer.concat([Buffer.from('RIFF'), Buffer.from([0, 0, 0, 0]), Buffer.from('WEBP'), Buffer.from('rest')]);
...
test('valid PDF accepted', () => {
  const r = validateFile({ filename: 'dimissione.pdf', declaredMime: 'application/pdf', data: PDF }, FILE_LIMIT);
...
test('valid image (jpg) accepted', () => {
  const r = validateFile({ filename: 'foto.jpg', declaredMime: 'image/jpeg', data: JPEG }, FILE_LIMIT);
  assert.equal(r.ok, true);
  assert.equal(r.file?.mimeType, 'image/jpeg');
});

test('DOCX (zip container) accepted only with doc/docx extension', () => {
  const ok = validateFile({ filename: 'lettera.docx', declaredMime: 'application/octet-stream', data: ZIP_DOCX }, FILE_LIMIT);
  assert.equal(ok.ok, true);
  const bareZip = validateFile({ filename: 'archivio.zip', declaredMime: 'application/zip', data: ZIP_DOCX }, FILE_LIMIT);
...
test('EXE renamed to .pdf is rejected by magic bytes', () => {
  const r = validateFile({ filename: 'malware.pdf', declaredMime: 'application/pdf', data: EXE }, FILE_LIMIT);
  assert.equal(r.ok, false);
  assert.equal(r.reason, 'type_not_allowed');
});

test('forged extension mismatch (pdf bytes as .png) rejected', () => {
  const r = validateFile({ filename: 'fake.png', declaredMime: 'image/png', data: PDF }, FILE_LIMIT);
  assert.equal(r.ok, false);
  assert.equal(r.reason, 'extension_mismatch');
});

test('oversize file rejected', () => {
  const r = validateFile({ filename: 'big.pdf', declaredMime: 'application/pdf', data: PDF }, { maxFileBytes: 4 });
  assert.equal(r.ok, false);
  assert.equal(r.reason, 'too_large');
});

test('empty file rejected', () => {
  const r = validateFile({ filename: 'empty.pdf', declaredMime: 'application/pdf', data: Buffer.alloc(0) }, FILE_LIMIT);
  assert.equal(r.ok, false);
  assert.equal(r.reason, 'empty');
});

test('identical bytes produce identical sha256 (dedup key)', () => {
  const a = validateFile({ filename: 'a.pdf', declaredMime: 'application/pdf', data: PDF }, FILE_LIMIT);
  const b = validateFile({ filename: 'b.pdf', declaredMime: 'application/pdf', data: Buffer.from(PDF) }, FILE_LIMIT);
