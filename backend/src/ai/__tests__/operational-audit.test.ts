...
const ALLOWED_KEYS = new Set([
  'requestId', 'operatorId', 'operatorRole', 'patientId', 'actionType', 'kind', 'channel', 'fields', 'outcome', 'createdAt',
]);
...
async function capture(fn: () => void): Promise<AiAuditEventInput[]> {
  ...
  try { fn(); await Promise.resolve(); } finally { setAuditPersistence(null); }
  return events;
}
...
test('#223 AC1: records actor / action / entity / outcome / timestamp with channel ui', async () => {
  const events = await capture(() =>
    recordOperationalAudit({
      requestId: 'req-1', actorId: 'op7', actorRole: 'infermiere',
      action: 'consegna.update', kind: 'update', patientId: 'pat-9',
      fields: ['stato'], outcome: 'ok', at: '2026-07-06T08:00:00.000Z',
    }),
  );
...
test('#223 AC2: only field NAMES are recorded — no values / payload / secrets can be passed', async () => {
  const events = await capture(() =>
    recordOperationalAudit({ actorId: 'op1', action: 'patient.update_contact', fields: ['telefono', 'email'] }),
  );
  const e = events[0];
...
test('#223 AC2: defaults are safe (role operatore, kind update, outcome ok, channel ui, fields [])', async () => {
  const events = await capture(() => recordOperationalAudit({ actorId: 'op1', action: 'room.assign' }));
  const e = events[0];
