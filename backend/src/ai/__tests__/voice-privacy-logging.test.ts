...
async function captureLogs(fn: () => Promise<void>): Promise<string[]> {
  const lines: string[] = [];
  const push = (...args: unknown[]) => { lines.push(args.map((a) => (typeof a === 'string' ? a : JSON.stringify(a))).join(' ')); };
  const orig = { log: console.log, error: console.error, warn: console.warn, info: console.info, debug: console.debug };
  console.log = push; console.error = push; console.warn = push; console.info = push; console.debug = push;
  try {
    await fn();
  } finally {
...
const ALLOWED_AUDIT_KEYS = new Set([
  'requestId', 'userId', 'patientId', 'actionType', 'recordId', 'fields', 'source', 'channel', 'outcome', 'at',
]);
...
test('#202 AC1/AC2: confirmed create — transcript & audit are sanitized; audit keys are minimal', async () => {
  ...
  setAuditPersistence(async (evt) => { persisted.push(evt); });
  const { w, calls } = fakeWriter();
  try {
    const lines = await captureLogs(async () => {
      const r = await executeCommand(
        { text: TRANSCRIPT, channel: 'voce', patientId: PID, idempotencyKey: 'k-202-vital', confirmed: true, operatorCtx },
        execDeps(w),
      );
    ...
    for (const k of Object.keys(entry)) assert.ok(ALLOWED_AUDIT_KEYS.has(k), `chiave audit non consentita: ${k}`);
    ...
    for (const f of entry.fields as unknown[]) {
      ...
      assert.ok(!/\s/.test(f as string), `un field name non deve contenere spazi (sospetto valore): ${f}`);
      assert.ok(!(f as string).includes('137') && !(f as string).includes('82'), `un field name non deve contenere un valore vitale: ${f}`);
    }
    ...
    assert.deepEqual(persisted[0].fields.filter((f) => /\s/.test(f)), [], 'nessun field persistito deve contenere spazi');
  } finally {
...
test('#202 AC1/AC2: free-text diary note content never appears in logs or persisted audit', async () => {
  ...
  try {
    const lines = await captureLogs(async () => {
      const r = await executeCommand(
        { text: TRANSCRIPT, channel: 'voce', patientId: PID, idempotencyKey: 'k-202-diary', confirmed: true, operatorCtx },
        execDeps(w),
      );
      assert.equal(r.ok, true);
  ...
  try {
    const lines = await captureLogs(async () => {
      await assert.rejects(
        () => executeCommand(
          { text: TRANSCRIPT, channel: 'voce', patientId: PID, idempotencyKey: 'k-202-del', confirmed: true, operatorCtx },
          execDeps(w),
        ),
    ...
    for (const evt of refusals) {
      ...
      assert.ok(!JSON.stringify(evt).toLowerCase().includes('mariazzrossi'), 'PHI nell\'evento di rifiuto');
    }
  } finally {
    setAuditPersistence(null);
  }
...
test('#202 AC1: plan-time delete refusal emits no transcript', async () => {
  ...
  const lines = await captureLogs(async () => {
    const r = await planCommand({ text: TRANSCRIPT, channel: 'voce', currentPatientId: PID, operatorCtx }, { loadPreviewContext: async () => ({}) });
