...
export type AuditAction =
  | 'job_created' | 'files_added' | 'files_removed' | 'process_started'
  | 'process_completed' | 'process_failed' | 'job_cancelled'
  | 'confirm_started' | 'patient_created' | 'confirm_committed'
  | 'duplicate_flagged' | 'confirm_failed';
...
export async function recordAudit(
  ...
  const detail = [opts.operatorId ? `op:${opts.operatorId}` : '', opts.detail ?? '']
    .filter(Boolean)
    .join(' ')
    .slice(0, 200) || null;
  try {
