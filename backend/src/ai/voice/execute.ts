...
export async function executeAction(
  ...
  if (!cfg.voiceEnabled) {
    audit(null, 'denied');
    throw new VoiceError('feature_disabled', 'Funzione vocale disabilitata.');
  }
  if (!isWriteAction(plan.actionType)) {
    audit(null, 'denied');
    throw new VoiceError('not_executable', 'Questa azione non è eseguibile come scrittura.');
  }
  if (!cfg.writeActionsEnabled) {
    audit(null, 'denied');
    throw new VoiceError('writes_disabled', 'Le azioni di scrittura sono disabilitate.');
  }
  if (!plan.patientId) {
    audit(null, 'denied');
    throw new VoiceError('ambiguous', 'Paziente non identificato.');
  }
  if (plan.ambiguities.length > 0) {
    audit(null, 'denied');
    throw new VoiceError('ambiguous', plan.ambiguities.join(' · '));
  }
  ...
  switch (plan.actionType) {
    ...
    case 'update_patient_demographics':
      recordId = await writer.updateDemographics(
        plan.patientId,
        String(plan.fields.field),
        String(plan.fields.value),
        meta,
      );
      break;
    case 'update_narrative_section':
      recordId = await writer.appendNarrative(
        plan.patientId,
        String(plan.sectionKey),
        String(plan.fields.addedText),
        meta,
      );
      break;
    case 'add_diary_note':
      recordId = await writer.addDiaryNote(plan.patientId, String(plan.fields.content), meta);
      break;
    case 'create_appointment':
      recordId = await writer.createAppointment(plan.patientId, plan.fields, meta);
      break;
    case 'update_appointment':
      if (!plan.targetRecordId) {
        audit(null, 'denied');
        throw new VoiceError('ambiguous', 'Appuntamento da modificare non identificato.');
      }
