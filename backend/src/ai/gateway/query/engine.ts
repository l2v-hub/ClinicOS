...
function dateWindowBounds(v: unknown): { from?: Date; to?: Date } {
  ...
  if (typeof w.lastDays === 'number') {
    const to = new Date();
    const from = new Date(to.getTime() - w.lastDays * 864e5);
    return { from, to };
  }
  if (w.day === 'today' || w.day === 'yesterday') {
    const d = new Date();
    if (w.day === 'yesterday') d.setDate(d.getDate() - 1);
  ...
  if (w.from || w.to)
    return { from: w.from ? new Date(w.from) : undefined, to: w.to ? new Date(w.to) : undefined };
  return {};
}
...
function opToPrisma(f: RawFilter): unknown {
  switch (f.op) {
    ...
    case 'dateWindow': {
      const { from, to } = dateWindowBounds(f.value);
...
function sourceFor(entityName: string, row: Record<string, unknown>): SourceReference {
  ...
  return {
    sourceType: SRC_TYPE[entityName] ?? 'PATIENT_FIELD',
    patientId,
    recordId,
    label: entityName,
  };
...
function projectRows(rows: Record<string, unknown>[], select: string[]): unknown[] {
  ...
  return rows.map((r) => {
    ...
    for (const k of select) {
      ...
      o[k] = sub ? (r[rel] as Record<string, unknown> | undefined)?.[sub] : r[k];
    }
    return o;
  });
}
...
async function runStep(
  ...
  for (const s of step.select) {
    const [rel, sub] = s.split('.');
    if (sub) relSet.add(rel);
  }
  ...
  return {
    rows: projectRows(rows, step.select),
    sources: rows.map((r) => sourceFor(step.entity, r)),
  };
}
  ...
  for (const original of plan.steps) {
    ...
    if (step.bindFrom) {
      ...
      const vals = (prev?.rows ?? [])
        .map((r) => (r as Record<string, unknown>)[step.bindFrom!.field])
        .filter((v) => v != null);
      step = {
        ...step,
