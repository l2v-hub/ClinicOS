...
export function sniffAllowedMime(buf: Buffer): string | null {
  ...
  if (buf.length >= 12 && buf.toString('ascii', 4, 8) === 'ftyp') {
    ...
    if (brand.startsWith('heic') || brand === 'heix' || brand === 'hevc' || brand === 'hevx')
      return 'image/heic';
    if (brand.startsWith('mif1') || brand.startsWith('msf1') || brand === 'heif')
      return 'image/heif';
  }
  return null;
}
...
router.post(
  ...
  async (req, res) => {
    try {
      const file = (
        req as unknown as { file?: { originalname: string; mimetype: string; buffer: Buffer } }
      ).file;
      if (!file) {
      ...
      if (!ALLOWED_MIME.has(file.mimetype)) {
        res.status(415).json({ error: 'Tipo file non supportato (solo immagini o PDF)' });
        return;
      }
...
router.get(
  '/:patientId/documents/:documentId/content',
  requirePatientDocumentAccess,
  async (req, res) => {
    try {
