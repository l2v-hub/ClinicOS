...
export async function renderAssessmentPdf(snapshot: AssessmentSnapshot): Promise<Buffer> {
  ...
  const newPage = () => {
    ...
    y = height - margin;
    page.drawText('PAINAD - Valutazione del dolore non verbale', {
      x: margin,
      y,
      size: 14,
      font: bold,
    ...
    page.drawText(`Valutazione: ${dateTime(snapshot.assessedAt)} | PAINAD italiana, versione 1`, {
      x: margin,
      y,
      size: 9,
      font: regular,
  ...
  doc.setTitle('PAINAD - Valutazione finalizzata');
  doc.setProducer(`ClinicOS ${ASSESSMENT_RENDERER_VERSION}`);
  doc.setKeywords([
