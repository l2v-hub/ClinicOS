  ...
  return assessmentTransaction(async (tx) => {
    ...
    await tx.patientDocument.create({
      data: {
        ...
        originalName: `${row.type === 'painad' ? 'PAINAD' : row.type === 'gds15' ? 'GDS15' : row.type === 'mna' ? 'MNA' : row.type === 'tinetti' ? 'Tinetti' : 'Trasferimenti'}_${row.assessedAt.toISOString().slice(0, 10)}_${job.id}.pdf`,
