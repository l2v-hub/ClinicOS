...
- The save payload shape sent to `onUpdateCartella` is **identical** to today. `ora` and `firmaIpM` are still in the payload — only their _source_ changes (synthesised, not typed).
