...
- [ ] T013 [P] [US1] In `frontend/src/app-additions.css` delete every selector that introduces a `border`, `border-radius > 0`, filled `background-color`, or non-zero `box-shadow` on `.page-tabs__btn` or `.page-tabs__btn--active`. Annotate each _retained_ override (if any) with a single-line comment naming the FR it preserves
