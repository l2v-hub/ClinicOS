    ...
    terapie: [
      {
        id: uid(),
        tipo: 'farmacologica',
        descrizione: 'Schema insulinico: 100-150→3U, 151-200→5U, 201-250→7U, >250→avvisare medico',
        dataInizio: today,
        stato: 'attiva',
        operatore: 'Test E2E',
...
async function step3_putCartella() {
  ...
  if (status === 200) {
    ...
    fail(
      'Cartella clinica PUT',
      'Route not deployed. Backend needs redeployment with latest routes.',
    );
...
async function step4_verifyCartella() {
  ...
  const checks: [string, string, (v: unknown) => boolean][] = [
    ...
    [
      ...
      (v) =>
        Array.isArray(v) &&
        v.length === 1 &&
        (v[0] as Record<string, unknown>).giorni &&
        ((v[0] as Record<string, unknown>).giorni as unknown[]).length === 5,
    ],
  ...
  missing(
    'Agenda appuntamenti',
    'No POST /appointments endpoint. Appointments require User/Operator IDs (auth needed). Suggest: POST /appointments',
  );
  missing(
    'Scale Tinetti',
    'No Tinetti model in schema or cartella type. Suggest: add to CartellaPaziente',
  );
