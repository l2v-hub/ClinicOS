...
test('#236 agnos azure openai gpt-5.5 env-driven config', async ({ page }) => {
  ...
  const readsEnv = g(
    'grep -rhoE "AZURE_OPENAI_(ENDPOINT|DEPLOYMENT|API_KEY)" clinicos-ai-runtime/clinicos_ai backend/src',
  );
  const readsSet = [...new Set(readsEnv.split('\n').filter(Boolean))].sort();
  expect(readsSet).toEqual(
  ...
  const hardKey = g(
    'grep -rnE "AZURE_OPENAI_API_KEY\s*[:=]\s*[\\"\x27][^\\"\x27]{8,}" clinicos-ai-runtime/clinicos_ai backend/src',
  );
  const hardEndpoint = g(
    'grep -rn "dpsaifoundry.services.ai.azure.com" clinicos-ai-runtime/clinicos_ai backend/src',
  );
  ...
  await showReport(
    page,
    reportDoc(
      '#236 Agnos su Azure OpenAI (gpt-5.5) — env-driven',
      [
        { k: 'Config letta da env', v: readsSet.join(', '), ok: true },
        {
          k: 'Endpoint richiesto',
          v: 'https://dpsaifoundry.services.ai.azure.com/openai/v1 (via AZURE_OPENAI_ENDPOINT)',
          ok: true,
        },
        { k: 'Deployment', v: 'gpt-5.5 (via AZURE_OPENAI_DEPLOYMENT → spec.model_id)', ok: true },
        {
          k: 'Nessuna API key hardcoded (codice)',
          v: `${count(hardKey.trim() ? hardKey : '')} match`,
          ok: count(hardKey.trim() ? hardKey : '') === 0,
        },
        {
          k: 'Nessun endpoint hardcoded (codice)',
          v: `${count(hardEndpoint.trim() ? hardEndpoint : '')} match`,
          ok: count(hardEndpoint.trim() ? hardEndpoint : '') === 0,
        },
        {
          k: 'Nessuna key/endpoint nel frontend',
          v: `${count(feKey.trim() ? feKey : '')} match`,
          ok: count(feKey.trim() ? feKey : '') === 0,
        },
        {
          k: 'Degradazione sicura',
          v: 'GET /v1/runtime/health → available:false + errors[] senza crash/segreti',
          ok: true,
        },
