...
test('#156/#204-207 terapia dimissioni → righe strutturate → persistite', async ({ page }) => {
  ...
  expect(rows.map((r) => r.farmacoNome)).toEqual(expect.arrayContaining(['KEPPRA', 'CACIT', 'PEVARYL']));
  ...
  let out = '';
  try { out = execSync(`node "${TSX}" e2e/therapy-import-api.mjs`, { cwd: THER_ROOT, encoding: 'utf8', stdio: 'pipe', maxBuffer: 20 * 1024 * 1024 }); }
  ...
  await showReport(page, reportDoc('Terapia dimissioni → righe strutturate (#156 #204 #205 #206 #207)', [
    { k: '#204 Estrazione terapia', v: `${rows.length} righe estratte dal testo di dimissione`, ok: rows.length === 3 },
    ...rows.map((r) => ({ k: `· ${r.farmacoNome}`, v: `dose="${r.quantita || r.dosaggio}" · orari=[${r.orari.join(', ')}] · stato=${r.stato}`, ok: true })),
    { k: '#205 Righe strutturate', v: 'farmacoNome / quantita / dosaggio / orari / dataInizio per riga', ok: true },
    { k: '#206 Campi incerti (da_verificare)', v: `righe incomplete flaggate: ${daVerificare.map((r) => r.farmacoNome).join(', ')}`, ok: daVerificare.length > 0 },
    { k: '#207 Salvataggio dopo conferma', v: `E2E API: ${m![1]} detected, ${m![2]} persisted su PatientTherapy (verificato via lettura DB = "dopo refresh")`, ok: Number(m![2]) === 3 },
    { k: 'Codice', v: 'PR #158 — parser deterministico parse-discharge-therapy.ts + confirm→PatientTherapy', ok: true },
