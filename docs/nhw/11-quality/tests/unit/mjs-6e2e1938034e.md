---
id: "test.repository.scripts.nhw.test.coverage-validator.test.mjs"
kind: "unit-test"
title: "coverage-validator.test.mjs"
status: "observed"
summary: "node-test unit test surface."
bounded_contexts: []
sources:
  - path: "scripts/nhw/test/coverage-validator.test.mjs"
    confidence: "observed"
relations:
  - type: "belongs-to"
    target: "project.repository-automation"
    evidence: "scripts/nhw/test/coverage-validator.test.mjs"
    confidence: "observed"
tags:
  - "test"
  - "unit"
  - "node-test"
last_verified:
  commit: "working-tree"
  inventory_hash: "2795bded959bb18b7093e7b78e09e009420c83bddf50a2eb3099c3d580a2fb30"
---

## Question Answered

What does `test.repository.scripts.nhw.test.coverage-validator.test.mjs` represent in ClinicOS?

## Canonical Definition

test.repository.scripts.nhw.test.coverage-validator.test.mjs is the canonical unit-test named coverage-validator.test.mjs.

## Inputs

Test source: `scripts/nhw/test/coverage-validator.test.mjs`.

## Outputs

Objective pass/fail evidence for the behavior encoded in the test.

## Dependencies

Framework: `node-test`; owning project: `project.repository-automation`.

## Side Effects

May create isolated fixtures or exercise local runtime behavior as defined by the test.

## Consumers

CI/CD, quality gates, maintainers, and autonomous QA agents.

## Invariants

A test is evidence only for assertions and execution paths it actually exercises.

## Failure Modes

Assertion failure, fixture failure, unavailable dependency, timeout, or environment mismatch.

## Evidence

- `scripts/nhw/test/coverage-validator.test.mjs`

## Related Knowledge

- `belongs-to` → `project.repository-automation`
