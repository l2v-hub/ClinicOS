...
const artifactRoot = resolve(root, 'artifacts/task-validation/po-14-gds/backend');
...
try {
  ...
  for (const file of files) {
    ...
    const child = spawn(process.execPath, ['--import', 'tsx', '--test', '--test-concurrency=1', file], {
      ...
      env: { ...process.env, DATABASE_URL: database.url, AUTH_MODE: 'demo', NODE_ENV: 'test', AI_VOICE_ENABLED: 'true', AI_WRITE_ACTIONS_ENABLED: 'true', GDS15_PDF_QA_DIRECTORY: resolve(artifactRoot,'pdf-qa'), MNA_PDF_QA_DIRECTORY: resolve(artifactRoot,'pdf-qa/mna-regression'), TINETTI_PDF_QA_DIRECTORY: resolve(artifactRoot,'pdf-qa/tinetti-regression'), ASSESSMENT_PDF_QA_DIRECTORY: resolve(artifactRoot,'pdf-qa/painad-regression'), TRANSFERS_PDF_QA_DIRECTORY: resolve(artifactRoot,'pdf-qa/transfers-regression') },
    });
