...
const specs=[['gds15-normal.pdf','gds15-normal',2],['gds15-long.pdf','gds15-long',6],['mna-regression/mna-full-normal.pdf','mna-full-normal',3],['mna-regression/mna-full-long.pdf','mna-full-long',7],['mna-regression/mna-screening-partial.pdf','mna-screening-partial',3],
...
for(const [pdf,name,pages] of specs) {
 ...
 if(name.startsWith('gds15')) {
  const snapshot=await json('pdf-qa/'+name+'.snapshot.json'), normalized=squash(text);
  assert.equal(snapshot.items.length,15);
  for(const item of snapshot.items) {
   assert(normalized.includes(squash(item.label)),name+': '+item.id);
   assert(normalized.includes(`Risposta: ${item.description} | Punti: ${item.score}`));
  }
  for(const key of ['instruction','screeningNote','provenance','reference']) assert(normalized.includes(squash(snapshot[key])),name+': '+key);
  assert(normalized.includes(`Totale GDS-15: ${snapshot.result.total} / 15 — ${snapshot.result.label}`));
  assert(normalized.includes('Interpretazione dello screening'));
  assert(normalized.includes('MMSE < 15'));
  assert(!text.includes('Totale PAINAD'));
  if(name==='gds15-normal') assert.equal(snapshot.result.total,10);
  else {
   assert.equal(snapshot.result.total,5);
   assert(normalized.includes(squash(snapshot.correctionReason)));
   assert(normalized.includes('Rettifica della valutazione'));
   const withoutFurniture=text.split(/\r?\n/).filter(line=>{
    const value=line.trim();
    return value!=='GDS-15 - Scala di depressione geriatrica' && value!==`${snapshot.patient.lastName} ${snapshot.patient.firstName}` && !value.startsWith('Valutazione:') && !value.startsWith('Valutazione finalizzata |');
   }).join('\n');
   assert(squash(withoutFurniture).includes(squash(snapshot.notes)),name+': complete notes retained');
   assert(snapshot.notes.split('\n').filter(Boolean).every(line=>text.includes(line)));
  }
 } else if(name.startsWith('mna')) {
  const snapshot=await json('pdf-qa/mna-regression/'+name+'.snapshot.json');
...
const names=[...specs.map(row=>row[0]),...specs.filter(row=>row[1].startsWith('mna') || row[1].startsWith('gds15')).map(row=>row[0].replace('.pdf','.snapshot.json')),
...
await writeFile(resolve(qa,'visual-qa.json'),JSON.stringify({sourceTreeSha256:tests.sourceTreeSha256,recordedAt:new Date().toISOString(),
 allPagesInspected:true,noClippingOrOverlap:true,totalPages:39,pages:specs.map(([file,name,pages])=>({file,name,pages})),
 observations:[
  'All 39 final PNG pages were individually inspected with view_image: headers, identity, body, source text and page numbering remain readable without clipping or overlap.',
  'GDS15 normal: two pages, all fifteen frozen questions and Sì responses with individual points, total10/severe screening interpretation, original instruction, cognitive limitation, source and bibliographic reference.',
  'GDS15 long: six pages, all No responses total5/none, frozen correction link/reason and all4000 note codepoints including supported Unicode and explicit newlines. Snapshot source notices remain complete.',
...
console.log(JSON.stringify({visualQaRecorded:true,pages:39,files:files.length,sourceTreeSha256:tests.sourceTreeSha256}));
