...
assert.equal(sourceTreeSha256, (await json('gds15-test-source-manifest.json')).treeSha256);
...
for (const [name,command,args] of commands) {
  ...
  const result=spawnSync(command,args,{cwd:root,windowsHide:true,encoding:'utf8',env:{...process.env,DATABASE_URL:'postgresql://postgres@127.0.0.1:1/po14_validate_only'}});
...
const fontSource=resolve(root,'artifacts/task-validation/po-13-mna/backend/font-provenance.json');
...
await writeFile(resolve(artifact,'validation-checks.json'),JSON.stringify({startedAt,completedAt:new Date().toISOString(),sourceTreeSha256,
  ...
  build:{compiled:true,fontsCopiedAndByteVerified:true,prismaGenerationSkipped:'Prisma model schema unchanged; previously byte-verified private PO13 runtime retained.'},
