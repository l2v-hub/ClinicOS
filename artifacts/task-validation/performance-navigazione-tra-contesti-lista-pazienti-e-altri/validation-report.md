...
- Commit: (vedi `git log -1` sul branch `perf/navigation-instant`, base `origin/codex/subtle-dashboard-notifications` @367c560d)
- Date: 2026-09-25
...
2. **Navigazione in transition** — `pushNav`/`popstate` commitano `navKey` dentro
   `startTransition`: la pagina corrente resta visibile finché la nuova è pronta.
3. **Stale-while-revalidate** — nuova `lib/sessionCache.ts` (memoria di sessione, svuotata al
   ...
   - tab cartella: Terapia, Diario, Sezioni cliniche, catalogo Moduli e Parametri multipaziente
     ...
     quattro tab all'apertura della scheda;
...
- `frontend/src/lib/sessionCache.ts` (nuovo), `frontend/src/lib/patientTabSnapshots.ts` (nuovo),
  `frontend/src/lib/patientDetailPrefetch.ts` (nuovo), `frontend/src/lib/cachedFetch.ts`
...
| AC  | Result | Evidence                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| --- | -----: | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| AC1 |   PASS | `perf/comparison.md` (da `final-preview.json`): visite successive — mediana **70 ms**, massimo **99 ms**, 0/19 oltre 300 ms (baseline: 471 ms / 1326 ms / 15/19).                                                                                                                                                                                                                                                                                                             |
| AC2 |   PASS | `final-preview.json`: 0 chunk JS scaricati durante le 34 navigazioni (baseline 41); `screenshots/evidence-ac3.json`: 0 occorrenze di "Caricamento modulo…" dopo il login su tutti i contesti + scheda + tab. Navigazioni oltre 300 ms: **6/34** (baseline 27/34), tutte prime letture di dati entro ~1 s dall'apertura (Terapia per contratto, Parametri, tab appena aperti).                                                                                                 |
| AC3 |   PASS | `perf/evidence-ac3.mjs` → `screenshots/evidence-ac3.json`: ritorno su Pazienti disegnato da cache in **77 ms** senza "Caricamento…", nome precedente visibile, nome rinominato dallo stub visibile dopo **356 ms** (`ac3-1/2/3-*.png`).                                                                                                                                                                                                                                       |
...
| ---------------- | -----: | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Unit             |   PASS | `sessionCache.test.ts` (3), `navigationStaleWhileRevalidate.test.ts` (6): 9/9 verdi; suite completa in `logs/frontend-node-tests.log` (784 pass, 9 fail pre-esistenti identici alla baseline) |
| Integration      |     NA |                                                                                                                                                                                               |
...
| ----------------------------------------- | ------------: | ---------: |
| tutte le navigazioni — mediana            |        532 ms |      73 ms |
| tutte le navigazioni — media              |        627 ms |     140 ms |
| tutte le navigazioni — oltre 300 ms       |         27/34 |       6/34 |
| prime visite — mediana                    |        779 ms |      82 ms |
| visite successive — mediana / massimo     | 471 / 1326 ms | 70 / 99 ms |
| chunk JS scaricati durante le navigazioni |            41 |          0 |

Cicli intermedi: `cycle1-preview.json` (SWR + transition + preload), `cycle2` (prefetch lista,
hover cartella, cache tab), `cycle3/3b` (prefetch tab, directory), `final-preview.json`.

Con preflight CORS in cache lato backend (`Access-Control-Max-Age`, simulato con
`PREFLIGHT_MAX_AGE=600` nello stub): `final-preview-preflight-cached.json` — massimo 763 → 591 ms,
media "dati completi" 545 → 434 ms. Raccomandazione backend, non implementata (vincolo
...
- Misure su stub locale (dati sintetici, latenza fissa 200 ms, HTTP/1.1): la produzione (Railway,
  HTTP/2, dati reali) non è stata rimisurata dopo la modifica — serve un deploy e una verifica
  autenticata (prod è dietro Entra/OIDC).
- Restano ≥ 1 giro di rete: prima apertura della pagina Terapia (per contratto non precaricabile),
  prima apertura di Parametri, primo click su un tab della cartella entro ~1 s dall'apertura.
