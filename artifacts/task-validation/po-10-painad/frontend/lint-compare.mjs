...
// Moving unchanged source shifts hook references embedded in diagnostic prose.
// Keep original diagnostics in the receipt; normalize only those location numbers for matching.
const signature = (messages) => messages.map(({ ruleId, severity, title }) => `${ruleId}:${severity}:${title.replace(/\(at line \d+\)/g, '(at line <location>)')}`).sort();
