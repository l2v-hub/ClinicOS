...
import {startPo10Fixture,fixtureActors} from '../../../../tests/fixtures/po10-assessments.mjs';
const root=process.cwd(),folder=resolve('artifacts/task-validation/po-10-painad/preview'),port=4191;
const fixture=await startPo10Fixture();const {prisma,database}=fixture;
process.env.VITE_API_URL=`http://127.0.0.1:${port}/api`;
await database.db.query(`CREATE TABLE "__Po10PdfFault" (id BOOLEAN PRIMARY KEY, armed BOOLEAN NOT NULL); INSERT INTO "__Po10PdfFault" VALUES(true,false);
CREATE FUNCTION po10_pdf_fault() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
 IF NEW."assessmentId" IS NOT NULL AND (SELECT armed FROM "__Po10PdfFault" WHERE id=true) THEN RAISE EXCEPTION 'Synthetic PDF persistence failure'; END IF; RETURN NEW; END $$;
CREATE TRIGGER po10_pdf_fault BEFORE INSERT ON "PatientDocument" FOR EACH ROW EXECUTE FUNCTION po10_pdf_fault();`);
...
app.post('/fixture/pdf-fault',async(req,res)=>{await database.db.query('UPDATE "__Po10PdfFault" SET armed=$1 WHERE id=true',[req.body.armed===true]);res.json({ok:true})});
...
const exportState=async()=>{
 ...
 const state={requests,assessments,documents:docs.map(({dataBase64,...doc})=>doc)};
...
const server=app.listen(port,'127.0.0.1',()=>console.log(`PO10 synthetic preview http://127.0.0.1:${port}`));
