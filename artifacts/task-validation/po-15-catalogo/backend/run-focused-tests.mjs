...
const artifactRoot = resolve(root, 'artifacts/task-validation/po-15-catalogo/backend');
...
const files = selected.length ? selected : [
  'backend/src/assessments/__tests__/catalog-db.test.ts',
  'backend/src/patients/__tests__/cartella-nrs-db.test.ts',
  'backend/src/ai/__tests__/intake-nrs-db.test.ts',
  'backend/src/intake/__tests__/patient-review-db.test.ts',
  'backend/src/assessments/__tests__/gds15.test.ts',
  'backend/src/assessments/__tests__/gds15-db.test.ts',
  'backend/src/assessments/__tests__/gds15-sql-db.test.ts',
  'backend/src/assessments/__tests__/gds15-pdf-db.test.ts',
  'backend/src/assessments/__tests__/mna.test.ts',
  'backend/src/assessments/__tests__/mna-db.test.ts',
  'backend/src/assessments/__tests__/mna-sql-db.test.ts',
  'backend/src/assessments/__tests__/mna-pdf-db.test.ts',
  'backend/src/assessments/__tests__/tinetti.test.ts',
  'backend/src/assessments/__tests__/tinetti-db.test.ts',
  'backend/src/patients/__tests__/cartella-tinetti-db.test.ts',
  'backend/src/assessments/__tests__/tinetti-pdf-db.test.ts',
  'backend/src/ai/__tests__/intake-tinetti-db.test.ts',
  'backend/src/ai/__tests__/intake-confirm.test.ts',
  'backend/src/assessments/__tests__/painad.test.ts',
  'backend/src/assessments/__tests__/transfers.test.ts',
  'backend/src/assessments/__tests__/service-db.test.ts',
  'backend/src/assessments/__tests__/pdf-db.test.ts',
  'backend/src/assessments/__tests__/transfers-pdf-db.test.ts',
  'backend/src/assessments/__tests__/attestations-db.test.ts',
];
...
try {
  ...
  for (const file of files) {
    ...
    const child = spawn(process.execPath, ['--import', 'tsx', '--test', '--test-concurrency=1', file], {
      ...
      env: { ...process.env, DATABASE_URL: database.url, AUTH_MODE: 'demo', NODE_ENV: 'test', AI_VOICE_ENABLED: 'true', AI_WRITE_ACTIONS_ENABLED: 'true', GDS15_PDF_QA_DIRECTORY: resolve(artifactRoot, 'regression-output'), MNA_PDF_QA_DIRECTORY: resolve(artifactRoot, 'regression-output'), TINETTI_PDF_QA_DIRECTORY: resolve(artifactRoot, 'regression-output'), ASSESSMENT_PDF_QA_DIRECTORY: resolve(artifactRoot, 'regression-output'), TRANSFERS_PDF_QA_DIRECTORY: resolve(artifactRoot, 'regression-output') },
    });
