...
// Synthetic print-style inspection only: captures the real selected legacy DOM.
// The native print dialog and physical pagination remain outside this harness.
window.print=()=>{
 const clone=document.documentElement.cloneNode(true) as HTMLElement;
 clone.querySelectorAll('script').forEach(node=>node.remove());
 const style=document.createElement('style');
 style.textContent=[...document.styleSheets].flatMap(sheet=>[...sheet.cssRules])
  .filter(rule=>rule instanceof CSSMediaRule&&rule.conditionText==='print')
  .flatMap(rule=>[...(rule as CSSMediaRule).cssRules].map(child=>child.cssText)).join('\n');
 clone.querySelector('head')!.append(style);
 document.getElementById('po12-print-capture')?.remove();
 const capture=document.createElement('template');capture.id='po12-print-capture';
 capture.innerHTML='<!doctype html>'+clone.outerHTML;document.body.append(capture);
};
function Preview(){
 ...
 return <>
 <header className="no-print" style={{background:'#dce8f5',padding:12}}><strong>PO12 · solo pazienti sintetici</strong>
 ...
 <button onClick={()=>void fixture('fault',{mode:'attest-lost'}).then(()=>setMessage('Prossima risposta conferma persa'))}>Perdi risposta conferma</button>
 ...
 <button onClick={()=>void fixture('pdf-fault',{armed:true}).then(()=>setMessage('PDF bloccato nel database sintetico'))}>Simula errore PDF</button>
 <button onClick={()=>void fixture('pdf-fault',{armed:false}).then(()=>setMessage('PDF ripristinato'))}>Ripristina PDF</button>
 <button onClick={()=>void fixture('rename',{patientId}).then(()=>setRefresh(n=>n+1))}>Cambia nome sintetico</button>
 ...
 {data&&actor?<PatientDetail key={`${profile}:${patientId}`} paziente={data.patient} cartella={data.chart} initialTab={'tinetti' as any} assessmentDraftStore={store} consegne={[]} consegneSummary={null} consegneLoading={false} consegneError={null} consegneHasMore={false} onLoadMoreConsegne={noop} onRetryConsegne={noop} operatori={[]} camere={[]} camereLoadState="ready" camereLoadError={null} onRetryCamere={noop} canAssignRooms={false} onBack={noop} onAddConsegna={async()=>({kind:'failed',message:'Fuori scenario',uncertain:false}) as any} onUpdateConsegnaStato={noop} onUpdateCartella={async()=>false} onUpdatePaziente={async()=>false} onAssignCamera={async()=>({ok:false})} operatoreNome={profile==='operator'?'Operatrice QA':'Responsabile QA'} operatoreId={actor.id} operatoreRole={actor.role}/>:<p>Caricamento cartella sintetica…</p>}
 </main></div></div></>;
}
