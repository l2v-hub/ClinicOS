...
import {startPo12Fixture,fixtureActors} from '../../../../tests/fixtures/po12-tinetti.mjs';
const root=process.cwd(),folder=resolve('artifacts/task-validation/po-12-tinetti/preview'),port=4193;
const fixture=await startPo12Fixture();const {prisma,database}=fixture;
const initialCartelle=await prisma.cartella.findMany();
process.env.VITE_API_URL=`http://127.0.0.1:${port}/api`;
await database.db.query(`CREATE TABLE "__Po12PdfFault" (id BOOLEAN PRIMARY KEY, armed BOOLEAN NOT NULL); INSERT INTO "__Po12PdfFault" VALUES(true,false);
CREATE FUNCTION po12_pdf_fault() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
 IF NEW."assessmentId" IS NOT NULL AND (SELECT armed FROM "__Po12PdfFault" WHERE id=true) THEN RAISE EXCEPTION 'Synthetic PDF persistence failure'; END IF; RETURN NEW; END $$;
CREATE TRIGGER po12_pdf_fault BEFORE INSERT ON "PatientDocument" FOR EACH ROW EXECUTE FUNCTION po12_pdf_fault();`);
...
app.post('/fixture/pdf-fault',async(req,res)=>{await database.db.query('UPDATE "__Po12PdfFault" SET armed=$1 WHERE id=true',[req.body.armed===true]);res.json({ok:true})});
...
const server=app.listen(port,'127.0.0.1',()=>console.log(`PO12 synthetic preview http://127.0.0.1:${port}`));
