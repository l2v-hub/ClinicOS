...
const expectedRoot = 'C:/Workspace/ClinicOSHouse-worktrees/po12-tinetti-backend';
const root = await realpath(process.cwd());
const baseline = '470a5fe7a7eee4a9b0a0b219947432049b9de977';
const expectedBranch = 'codex/po12-tinetti-backend';
if (root !== await realpath(expectedRoot)) throw new Error('Wrong worktree');
const artifact = resolve(root, 'artifacts/task-validation/po-12-tinetti/backend');
const previous = await realpath('C:/Workspace/ClinicOSHouse-worktrees/po11-transfers-backend');
...
const sourceScope = ['backend/src','prisma','frontend/src','scripts','tests/fixtures','backend/tsconfig.json','backend/package.json','package.json','package-lock.json','prisma.config.ts'];
...
const contractPath = resolve(root,'artifacts/task-validation/po-12-tinetti/task-contract.md');
...
await writeFile(resolve(artifact,'preparation-session.json'),JSON.stringify({
  task:'PO12-backend-preparation', startedAt:new Date().toISOString(), root, branch:expectedBranch, baseline,
  decision:'allow private runtime copy and preparation artifacts only; application implementation awaits explicit root GO after PO11 deploy',
...
if(!Prisma.dmmf.datamodel.models.some(model=>model.name==='PatientAssessmentAttestation')) throw new Error('PO11 attestation model missing');
...
const receipt = {
  task:'PO12-backend-preparation',phase:'private-runtime-ready-awaiting-implementation-go',preparedAt:new Date().toISOString(),
  ...
  recall:{memoryMatches:4,relevantMatches:0,reference:'preparation-recall.json; PO11 backend implementation receipt and read-only task contract'},
  implementationAuthorized:false,publicationAuthorized:false,
  limitations:['Copied generated client is valid for the PAINAD/Transfers baseline only; regenerate exclusively to this private output after an authorized PO12 schema change.',
