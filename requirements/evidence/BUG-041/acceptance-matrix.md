# BUG-041 — Acceptance matrix

Issue: #63 — *Tipo e stato diagnosi inferiti senza evidenza*

## Triage result: ALREADY RESOLVED IN CODE (verify-on-deploy)

No diagnosis type/status fields are generated by the import — only faithful narrative text. Regression test asserts the persistence row carries no type/stato/status field.
