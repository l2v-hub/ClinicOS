...
const H = (extra = {}) => ({
  ...
  'X-AI-User-Id': 'u-test',
  'X-AI-Roles': 'operator',
  'X-AI-Request-Id': 'req-test',
  ...extra,
});
const post = (p, body, h) =>
  fetch(`${BASE}${p}`, { method: 'POST', headers: H(h), body: JSON.stringify(body) });
...
const run = async () => {
  ...
  const noTok = await fetch(`${BASE}/search/patients`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'X-AI-User-Id': 'u' },
    body: '{}',
  });
  ok(noTok.status === 401, 'no service token → 401');

  // 2. missing user context → 401
  const noUser = await fetch(`${BASE}/search/patients`, {
    method: 'POST',
  ...
  ok(
    srBody.length > 0 &&
      srBody.every((r) => Array.isArray(r.sourceRefs) && r.sourceRefs.length > 0),
    'every patient result has SourceReferences',
  );
  ...
  const forbidden = await post(
    ...
    { patientId: pid },
    { 'X-AI-Permitted-Patients': 'some-other-id' },
  );
  ...
  const demo = await post(
    '/patient/demographics',
    { patientId: pid },
    { 'X-AI-Permitted-Patients': pid },
  );
  const demoBody = await demo.json();
  ok(
    demo.status === 200 && demoBody?.sourceRefs?.length > 0,
    'demographics within allow-list → 200 + source',
  );
  ...
  ok(
    vit.status === 200 && Array.isArray(vitBody.data) && Array.isArray(vitBody.sourceRefs),
    'vital-signs → sourced envelope',
  );
  ...
  ok(
    tl.status === 200 && Array.isArray(tlBody.data) && Array.isArray(tlBody.sourceRefs),
    'timeline → sourced envelope',
  );
  ...
  ok(
    inj.status === 200,
    'SQL-injection-looking query is a harmless literal filter (200, no error)',
  );
  // the table still exists:
  const still = await (await fetch('http://localhost:3001/patients')).json();
  ok(
    Array.isArray(still) && still.length === pats.length,
