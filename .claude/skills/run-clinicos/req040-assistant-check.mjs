...
const H = (extra = {}) => ({
  ...
  'X-AI-User-Id': 'u-test',
  'X-AI-Roles': 'operator',
  'X-AI-Request-Id': 'req-040',
  ...extra,
});
const ask = (question, body = {}, h) =>
  fetch(`${BASE}/assistant/query`, {
    method: 'POST',
    headers: H(h),
...
const run = async () => {
  ...
  ok(
    a.status === 200 && a.body.intent === 'allergies' && Array.isArray(a.body.sources),
    'allergies → sourced answer',
  );
  ...
  ok(
    v.status === 200 && v.body.intent === 'vitals_recent' && Array.isArray(v.body.navigation),
    'recent vitals envelope',
  );
  ...
  const cross = await ask(
    'Trova i pazienti con valori pressori superiori a 150',
    {},
    { 'X-AI-Roles': 'manager' },
  );
  ok(
    cross.status === 200 && cross.body.intent === 'correlate' && !cross.body.refusal,
    'authorized cross-patient runs',
  );
  ...
  ok(
    denied.status === 200 && denied.body.refusal && denied.body.results.length === 0,
    'unauthorized cross-patient → refusal',
  );

  // 9. no-answer question (unknown) → notFound, nothing invented
  const none = await ask('Buongiorno come stai', { currentPatientId: pid });
  ok(
    none.status === 200 && none.body.notFound === true && none.body.results.length === 0,
    'unknown → not found, no invented data',
  );

  // 10. diagnosis request refused
  const diag = await ask('Che diagnosi ha questo paziente?', { currentPatientId: pid });
  ok(
    diag.status === 200 && diag.body.intent === 'refuse_clinical' && diag.body.refusal,
    'diagnosis request refused',
  );
  ...
  ok(
    al.body.results.length === 0 || al.body.sources.length > 0,
    'results carry sources (SOURCE_ONLY)',
  );
