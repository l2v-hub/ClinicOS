...
description: ClinicOS Backend / API agent — the only agent that edits backend/src and prisma. Use for Express routes, Prisma schema/migrations, DB queries and API contracts. Ensures data integrity, input validation and clean error handling; never touches backend/schema unless the task explicitly asks.
