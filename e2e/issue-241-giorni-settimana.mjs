...
try {
  ...
  ok('AC1 selettore giorni presente (tipo periodica)', await page.locator('[data-testid="therapy-weekdays"]').isVisible());
  for (const n of [1, 2, 4, 7]) {
    ok(`AC3 giorno ${n} selezionato (aria-pressed)`, (await page.locator(`[data-testid="weekday-${n}"]`).getAttribute('aria-pressed')) === 'true');
  }
  await shot('weekdays-selected.png');

  // Salva → POST con giorniSettimana
  const [resp] = await Promise.all([
    page.waitForResponse((r) => /\/patients\/.+\/therap/i.test(r.url()) && ['POST', 'PUT'].includes(r.request().method()), { timeout: 10000 }).catch(() => null),
    page.getByRole('button', { name: /Salva terapia/i }).click(),
  ]);
  await page.waitForTimeout(1500);
  ok('AC salvataggio therapy (POST/PUT 2xx)', !!resp && resp.status() >= 200 && resp.status() < 300, `status=${resp && resp.status()}`);
  ...
  ok('AC2/AC3 pill giorni mostra Lun…Dom', /Lun/.test(pillTxt) && /Dom/.test(pillTxt), `pill="${pillTxt}"`);
  ...
  ok('AC4 giorni persistono dopo reload', /Lun/.test(pill2Txt) && /Dom/.test(pill2Txt), `pill="${pill2Txt}"`);
  ...
  writeFileSync(join(OUT, 'ui-report.json'), JSON.stringify({ ranAt: new Date().toISOString(), patient: PATIENT, drug: DRUG, results, consoleErrors }, null, 2));
  await ctx.close();
  await browser.close();
}
