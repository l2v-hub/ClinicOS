...
async function newPage(browser, onRequest) {
  ...
  if (onRequest)
    page.on('request', (r) => {
      if (r.url().startsWith(BACKEND)) onRequest({ method: r.method(), url: r.url() });
    });
  return page;
}
...
async function perf(outFile) {
  ...
  flows['avvio-dashboard'] = bucket;
  bucket = [];

  await clickText(page, 'Agenda');
  await page.waitForLoadState('networkidle');
  await page.waitForTimeout(1500);
  flows['apertura-agenda'] = bucket;
  bucket = [];
  ...
  await page.waitForLoadState('networkidle');
  await page.waitForTimeout(1500);
  flows['dettaglio-paziente'] = bucket;
  bucket = [];
  ...
  for (const [name, reqs] of Object.entries(flows)) {
    report[name] = {
      total: reqs.length,
      duplicates: dupes(reqs),
      requests: reqs.map((r) => `${r.method} ${r.url}`),
    };
  }
  writeFileSync(
    outFile,
    JSON.stringify({ measuredAt: new Date().toISOString(), flows: report }, null, 2),
  );
  for (const [name, f] of Object.entries(report)) {
    console.log(
      `${name}: ${f.total} richieste, ${f.duplicates.length} duplicate ${f.duplicates.map((d) => `[${d.count}x ${d.request}]`).join(' ')}`,
    );
  }
  await browser.close();
}
...
async function us1(outDir) {
  ...
  await page.click('.voice-actions .btn-primary');
  await page
    .waitForSelector('.voice-done >> nth=1, .agnos-preview__stato--ok >> nth=1', { timeout: 10000 })
    .catch(() => {});
  ...
  ok(
    'read risponde senza conferma',
    /37,?2|temperatura/i.test(bodyTxt),
    '(risposta contiene il parametro)',
  );
  ...
  ok(
    'delete rifiutato con rinvio alla UI',
    /interfaccia/i.test(refusalTxt),
    refusalTxt.slice(0, 80),
  );
...
if (mode === 'perf') await perf(out ?? 'requirements/evidence/SPEC-015/perf-before.json');
  ...
  console.error('usage: spec015-verify.mjs perf|us1 [out]');
  process.exit(2);
}
