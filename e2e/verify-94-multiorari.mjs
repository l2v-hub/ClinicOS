...
try {
  ...
  page.on('response', (r) => {
    const m = r.url().match(/\/patients\/([^/]+)\/therapies/);
    if (m) patientId = m[1];
  });
  ...
  if (patientId) {
    ...
    report.allThreeTimes =
      !!mine[0] && times.every((t) => (mine[0].orarioSpecifico ?? '').includes(t));
    // cleanup
    for (const r of mine)
      await fetch(`${API}/patients/${patientId}/therapies/${r.id}`, { method: 'DELETE' });
  }
