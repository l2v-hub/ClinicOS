...
const avanti = async () => { await p.getByRole('button', { name: /Avanti|Conferma/i }).last().click(); await p.waitForTimeout(1100); };
...
await login();
await p.getByText('Nuovo paziente').first().click(); await p.waitForTimeout(1500);
...
ok('manuale: step Clinica renderizza senza crash', pageErrors.length === 0 && ((await p.textContent('body'))??'').length > 500);
await shot('after-step3-clinica.png');
await avanti(); // → 4 Moduli
await avanti(); // → 5 Documenti
await avanti(); // → 6 Verifica
await p.getByRole('button', { name: /Crea paziente|Conferma/i }).last().click();
await p.waitForTimeout(2500);
await shot('after-manual-created.png');
let pts = await (await fetch('http://localhost:3001/patients')).json();
ok('manuale: paziente persistito nel backend', pts.some(x => x.lastName === 'Verifica127'));
ok('manuale: paziente visibile in lista', ((await p.textContent('body'))??'').includes('Verifica127'));

// ── Flusso 2: importazione scansione ─────────────────────────────────────────
await login(); // stato pulito: il wizard manuale resta aperto dopo la creazione
await p.getByText('Importa dimissione').first().click(); await p.waitForTimeout(700);
await p.setInputFiles('input[type=file][multiple]', [fx.pdf, fx.jpg]); await p.waitForTimeout(1200);
...
await p.waitForTimeout(2500);
ok('import: workspace intake si apre senza crash', pageErrors.length === 0 && ((await p.textContent('body'))??'').length > 500);
await shot('after-import-workspace.png');
// completa il wizard seminato dall'import (anagrafica già presente)
for (let i = 0; i < 6; i++) {
  const fine = await p.getByRole('button', { name: /Crea paziente/i }).last().isVisible().catch(() => false);
  if (fine) { await p.getByRole('button', { name: /Crea paziente/i }).last().click(); await p.waitForTimeout(2500); break; }
  // step 2 richiede data presa in carico + motivo se vuoti
  const dt = p.locator('input[type="date"]').first();
  if (await dt.isVisible().catch(()=>false) && !(await dt.inputValue())) await dt.fill('2026-07-04');
  const ta = p.locator('textarea').first();
  if (await ta.isVisible().catch(()=>false) && !(await ta.inputValue())) await ta.fill('Import verifica 127');
  await avanti();
}
await shot('after-import-created.png');
pts = await (await fetch('http://localhost:3001/patients')).json();
ok('import: paziente persistito nel backend', pts.some(x => x.lastName === 'Verifica127B'));

// ── Persistenza dopo refresh ─────────────────────────────────────────────────
await login();
const body = (await p.textContent('body')) ?? '';
ok('persistenza dopo refresh: entrambi in lista', body.includes('Verifica127') && body.includes('Verifica127B'));
