...
const dir = resolve(tmpdir(), 'clinicos-req033');
mkdirSync(dir, { recursive: true });
const pdfPath = resolve(dir, 'lettera-dimissione-imola.pdf');
writeFileSync(
  pdfPath,
  buildSyntheticPdf(['AUSL Imola - Lettera di dimissione', 'Diagnosi: Scompenso cardiaco.']),
);
const svgPath = resolve(dir, 'foto.svg');
writeFileSync(
  svgPath,
  '<?xml version="1.0"?><svg xmlns="http://www.w3.org/2000/svg" width="700" height="950"><rect width="700" height="950" fill="#fff"/><rect width="700" height="70" fill="#0F5FD7"/><text x="20" y="44" font-size="22" fill="#fff" font-family="Arial">AUSL Imola</text></svg>',
);

const t = (raw, txt, tag, it) => {
  const i = raw.indexOf(txt);
  return { sectionKey: it, tagType: tag, text: txt, startOffset: i, endOffset: i + txt.length };
};
const diag =
...
const NARRATIVE = {
  ...
  diagnosisText: diag,
  anamnesisText: anam,
  hospitalCourseText: 'Il 12/03/2025 ingresso. Il 18/03/2025 dimissione.',
  consultationsText: 'Consulenza cardiologica del 13/03/2025.',
  imagingDiagnosticsText: 'Rx torace: congestione polmonare.',
  proceduresAndInterventionsText: 'Posizionamento CVC il 12/03/2025.',
  therapyText: ther,
  adviceAndFollowUpText: 'Controllo cardiologico tra 30 giorni. Dieta iposodica.',
  unmappedText: '',
  boldTags: [
    t(anam, 'Anamnesi patologica remota', 'SUBSECTION_TITLE', 'ANAMNESI'),
    t(anam, '12/03/2018', 'DATE', 'ANAMNESI'),
    t(allg, 'Pantoprazolo', 'ALLERGY_CRITICAL', 'ALLERGIE'),
    t(ther, 'Ramipril', 'MEDICATION_NAME', 'TERAPIA'),
  ],
  sourceReferences: [
    { sectionKey: 'DIAGNOSI', fileName: 'lettera-dimissione-imola.pdf', pageFrom: 1 },
...
const legacyDiagnosi = Array.from({ length: 36 }, (_, i) => ({
  codiceICD: `J44.${i}`,
  descrizione: `Diagnosi legacy ${i + 1}`,
  tipo: i === 0 ? 'principale' : 'comorbidita',
  stato: 'attiva',
}));
const RESULT = {
  ...
  _sections: null,
  rawText:
    'AUSL Imola - Lettera di dimissione\nDiagnosi: Scompenso cardiaco.\nTerapia: Ramipril. [ILLEGGIBILE] 1-0-1.',
  _full: {
    anagrafica: { nome: 'Mario', cognome: 'Bianchi' },
    cartella: { diagnosi: legacyDiagnosi, farmaci: [] },
  },
  _merge: { report: { conflict: 0 } },
};
const JOB = {
  id: 'j33',
  status: 'uploaded',
...
async function mock(p) {
  await p.route('**/*', async (r) => {
    ...
    if (u.includes('/ai/extraction/jobs')) {
      ...
      if (m === 'GET' && u.endsWith('/result'))
        return j({ status: 'review_ready', resultData: RESULT });
...
try {
  ...
  await p.setInputFiles('input[type=file][multiple]', [pdfPath, svgPath]);
  await p.waitForTimeout(700);
  await p.getByRole('button', { name: /Avvia elaborazione/i }).click();
  await p.waitForSelector('[data-testid="sections-review"]', { timeout: 15000 });
  await p.waitForTimeout(800);
  await p.screenshot({ path: resolve(out, 'wide-review-real-component.png') });
  await p
    .locator('[data-testid="srev-DISCHARGE_DIAGNOSIS"]')
    .screenshot({ path: resolve(out, 'after-runtime-diagnosis-text.png') });
  await p
    .locator('[data-testid="srev-ALLERGIES"]')
    .screenshot({ path: resolve(out, 'after-runtime-allergies-text.png') });
  await p
    .locator('[data-testid="srev-ANAMNESIS"]')
    .screenshot({ path: resolve(out, 'after-runtime-anamnesis-text.png') });
  await p
    .locator('[data-testid="srev-DISCHARGE_HOME_THERAPY"]')
    .screenshot({ path: resolve(out, 'after-runtime-therapy-text.png') });
