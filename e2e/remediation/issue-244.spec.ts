...
const EVIDENCE_DIR = path.resolve(
  __dirname,
  '../../artifacts/task-validation/244-allergies-absent-denied',
);
...
test('issue #244 — allergy status is unambiguous in the summary and cannot be set to a contradictory state', async ({
  page,
}) => {
  ...
  expect(cartellaPutRequests.length, 'expected a PUT when setting paziente_nega').toBeGreaterThan(
    0,
  );
  await closeModal();
  ...
  await page.waitForLoadState('networkidle');
  await openPatient();
  await expect(page.locator('[data-testid="allergy-summary-state"]')).toContainText(
    'Paziente nega allergie',
  );
  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, 'summary-paziente-nega.png'),
    fullPage: true,
  });
  ...
  await expect(page.locator('[data-testid="allergy-status-presenti"]')).toHaveAttribute(
    'aria-checked',
    'true',
  );
  ...
  await page.screenshot({
    path: path.join(SCREENSHOTS_DIR, 'blocked-assenti.png'),
    fullPage: true,
  });

  // ── Cleanup: remove the allergen, restore "paziente_nega" ───────────────────
  await page
    .locator('.ec-modal-item')
    .filter({ hasText: 'Penicillina (test #244)' })
    .locator('button[title="Elimina"]')
    .click();
