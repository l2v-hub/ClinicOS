...
async function openAllergieModal() {
  await page.getByRole('button', { name: /Allergie/i }).first().click();
  await page.waitForSelector('[data-testid="allergy-status"]', { timeout: 8000 });
}
async function setStatus(key) {
  const [resp] = await Promise.all([
    page.waitForResponse((r) => /\/patients\/.+\/cartella/.test(r.url()) && r.request().method() !== 'GET', { timeout: 8000 }).catch(() => null),
    page.locator(`[data-testid="allergy-status-${key}"]`).click(),
  ]);
  await page.waitForTimeout(800);
  return resp;
}
function closeModal() { return page.locator('.modal-close, [aria-label="Chiudi"]').first().click().catch(() => page.keyboard.press('Escape')); }
...
try {
  ...
  ok('selettore stato allergie visibile', await page.locator('[data-testid="allergy-status"]').isVisible());

  // AC2 — paziente nega
  const r1 = await setStatus('paziente_nega');
  ok('AC2 stato "paziente nega" → badge allergy-denied', await page.locator('[data-testid="allergy-denied"]').isVisible());
  ok('AC2 salvataggio persistito (PUT cartella)', !!r1 && r1.status() >= 200 && r1.status() < 300, `status=${r1 && r1.status()}`);
  ...
  await openPatient();
  await openAllergieModal();
  ok('AC4 "paziente nega" persiste dopo reload', await page.locator('[data-testid="allergy-denied"]').isVisible());
  await shot('after-refresh-nega.png');

  // AC1 — assenti
  const r2 = await setStatus('assenti');
  ok('AC1 stato "assenti" → badge allergy-none', await page.locator('[data-testid="allergy-none"]').isVisible());
  ok('AC1 salvataggio persistito (PUT cartella)', !!r2 && r2.status() >= 200 && r2.status() < 300, `status=${r2 && r2.status()}`);
  ...
  await openPatient();
  await openAllergieModal();
  ok('AC4 "assenti" persiste dopo reload', await page.locator('[data-testid="allergy-none"]').isVisible());
  ...
  ok('AC3 stato "presenti" selezionabile', await page.locator('[data-testid="allergy-status-presenti"]').getAttribute('aria-checked') === 'true', `r3=${r3 && r3.status()}`);
