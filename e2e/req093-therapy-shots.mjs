...
async function gotoTherapy(page) {
  ...
  if (await loginCard.count() > 0) { await loginCard.click(); await page.waitForTimeout(1000); }

  // Pazienti
  const pazienti = page.locator('.teams-sidebar__item').filter({ hasText: /Pazienti/i }).first();
  if (await pazienti.count() > 0) { await pazienti.click(); await page.waitForTimeout(1000); }
  else { await page.getByText('Pazienti').first().click(); await page.waitForTimeout(1000); }

  // open patient Forlano
  const row = page.getByText('Forlano').first();
  if (await row.count() > 0) { await row.click(); await page.waitForTimeout(1200); }

  // Clinica L2 tab (label shows "Clinica 7")
  const clinica = page.getByText(/^Clinica/).first();
  if (await clinica.count() > 0) { await clinica.click().catch(() => {}); await page.waitForTimeout(800); }
  // Terapia Farmacologica L3 tab
  const tab = page.getByText('Terapia Farmacologica', { exact: false }).first();
  if (await tab.count() > 0) { await tab.click().catch(() => {}); await page.waitForTimeout(1400); }
}
  ...
  try {
    ...
    if (suffix === 'desktop') {
      ...
      if (await giorn.count() > 0) {
        await giorn.click().catch(() => {}); await page.waitForTimeout(900);
        const dateInput = page.locator('input[type="date"]').first();
        if (await dateInput.count() > 0) { await dateInput.fill('2026-06-24'); await page.waitForTimeout(1200); }
