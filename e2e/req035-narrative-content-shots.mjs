...
const dir = resolve(tmpdir(), 'clinicos-req035');
mkdirSync(dir, { recursive: true });
const pdfPath = resolve(dir, 'lettera-imola.pdf');
writeFileSync(
  pdfPath,
  buildSyntheticPdf(['AUSL Imola', 'Anamnesi Patologica Recente: Inviata in PS in data 09/03.']),
);
...
const NARRATIVE = {
  ...
  allergiesText: 'Pantoprazolo: riferita reazione cutanea.',
  ...
  unmappedText: '',
  boldTags: [],
  sourceReferences: [{ sectionKey: 'ANAMNESI', fileName: 'lettera-imola.pdf', pageFrom: 2 }],
  missingSections: [],
  warnings: [],
};
const RESULT = {
...
try {
  ...
  await p.setInputFiles('input[type=file][multiple]', [pdfPath]);
  await p.waitForTimeout(700);
  ...
  console.log(
    'anamnesis card has extracted text:',
    (cardText || '').includes('Inviata in PS in data 09/03'),
  );

  // Modifica -> editor prefilled.
  await anam.getByRole('button', { name: 'Modifica' }).click();
  await p.waitForTimeout(400);
  const ta = anam.locator('textarea');
  const taVal = await ta.inputValue();
  console.log(
    'anamnesis editor prefilled startsWith heading:',
    taVal.startsWith('## Anamnesi Patologica Recente:'),
  );
  ...
  await diag.getByRole('button', { name: 'Modifica' }).click();
  ...
  await ther.scrollIntoViewIfNeeded();
  await ther.getByRole('button', { name: 'Modifica' }).click();
  await p.waitForTimeout(300);
  ...
  await anam.scrollIntoViewIfNeeded();
  await anam
    .getByRole('button', { name: /Confronta con la fonte/ })
    .click()
    .catch(() => {});
  await p.waitForTimeout(300);
