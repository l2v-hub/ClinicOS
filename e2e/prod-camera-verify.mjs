...
try {
  ...
  await card.screenshot({ path: resolve(outDir, `camera-${mode}-take-photo-button.png`) }).catch(() => {});
  ...
  if (mode === 'fake') {
    ...
    // capture → preview
    if (await page.locator('[data-testid="camera-shoot"]').count()) await page.locator('[data-testid="camera-shoot"]').click();
    ...
    if (await page.locator('[data-testid="camera-use"]').count()) await page.locator('[data-testid="camera-use"]').click();
