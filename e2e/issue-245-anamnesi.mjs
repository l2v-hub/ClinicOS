...
try {
  ...
  ok(
    'AC1/AC3 nessun tab "Anamnesi" duplicato nel gruppo Clinica',
    !hasAnamnesiTab,
    `hasAnamnesiTab=${hasAnamnesiTab}`,
  );
  ok(
    'unica superficie anamnesi presente: "Sezioni Cliniche (testo)"',
    hasNarrative,
  ...
  ok(
    'AC2 sezione clinica narrativa raggiungibile (contenuto renderizzato)',
    bodyLen > 200,
    `bodyChars=${bodyLen}`,
  );
  ...
  ok(
    'nessun NUOVO console error introdotto da #245',
    newErrors.length === 0,
    `nuovi=${newErrors.length} (preesistenti filtrati=${consoleErrors.length - newErrors.length})`,
  );
} finally {
