...
async function navTherapy(page) {
  ...
  if (await login.count() > 0) { await login.click(); await page.waitForTimeout(900); }
  const pazienti = page.locator('.teams-sidebar__item').filter({ hasText: /Pazienti/i }).first();
  if (await pazienti.count() > 0) { await pazienti.click(); await page.waitForTimeout(900); }
  const row = page.getByText('Forlano').first();
  if (await row.count() > 0) { await row.click(); await page.waitForTimeout(1100); }
  const clinica = page.getByText(/^Clinica/).first();
  if (await clinica.count() > 0) { await clinica.click(); await page.waitForTimeout(700); }
  ...
  else { await page.getByText('Terapia Farmacologica').first().click(); }
...
try {
  ...
  if (await editBtn.count() > 0) { await editBtn.click(); await page.waitForTimeout(900); }
  await page.screenshot({ path: resolve(outDir, 'after-form-editor-desktop.png'), fullPage: false });
  ...
  if (await giorn.count() > 0) { await giorn.click(); await page2.waitForTimeout(800); }
  const dateInput = page2.locator('input[type="date"]').first();
  if (await dateInput.count() > 0) { await dateInput.fill('2026-06-24'); await page2.waitForTimeout(1300); }
  await page2.screenshot({ path: resolve(outDir, 'after-administration-desktop.png'), fullPage: false });
