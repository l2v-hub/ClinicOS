...
async function ensurePatient(firstName, lastName) {
  ...
  for (const a of assigns)
    await fetch(`${API}/patients/${p.id}/room-assignments/${a.id}`, { method: 'DELETE' });
...
const target = roomsPre.find(
  (r) =>
    r.stato === 'attiva' &&
    r.beds.length === 1 &&
    r.beds.every((b) => b.assignments.length === 0 && b.stato !== 'manutenzione'),
);
if (!target) {
  console.error('Nessuna camera singola libera disponibile per il test');
  process.exit(1);
}
...
async function apriPaziente(lastName) {
  ...
  await p
    .getByText(new RegExp(`^${lastName},`))
    .first()
    .click();
  await p.waitForTimeout(1200);
}
async function apriModalCamera() {
  await p.locator('.cr-quick-stat--camera').click();
...
await p.getByText('Modifica assegnazione').click();
...
ok(
  'AC1: paziente associato al letto',
  bedOcc?.currentAssignment?.patient?.lastName === 'Camera128',
...
ok(
  'AC1: assegnazione attiva persistita per il paziente',
  assigns1.some((a) => a.endDate === null && a.bed?.room?.numero === target.numero),
);

// ── AC2: camera non più proposta come libera per un altro paziente ──────────
await p
  .getByText(/^(Pazienti|Torna|←)/)
  .first()
  .click()
  .catch(() => {});
await loginOperatore();
...
ok(
  'AC2: camera occupata NON proposta per un altro paziente',
  !options2.some((o) => o.startsWith(`${target.numero} `)),
...
ok(
  'AC3: dopo refresh la camera resta occupata (API)',
  Boolean(room2?.beds?.some((x) => x.occupied)),
);
await shot('after-refresh.png');

// ── AC4: coerenza tra viste (scheda paziente / Posti Letto admin) ────────────
await p.goto(FRONTEND, { waitUntil: 'networkidle' });
await p.getByText('Amministratore', { exact: true }).click();
await p.waitForTimeout(1200);
await p.getByText('Posti Letto').first().click();
await p.waitForTimeout(1500);
const postiTxt = (await p.textContent('body')) ?? '';
const occStat = await p
  .locator('.occupancy-stats')
  .textContent()
  .catch(() => '');
ok(
  'AC4: vista Posti Letto conta almeno 1 letto occupato',
  !/(^|\D)0\s*Occupati/.test(occStat ?? ''),
  (occStat ?? '').slice(0, 80),
);
