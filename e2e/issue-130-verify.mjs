...
const OP_HEADERS = { 'X-Operator-Id': 'e2e-130', 'X-Operator-Role': 'operatore', 'X-Operator-Name': 'E2E Issue130' };
...
async function dictate(page, transcript) {
  await page.evaluate((t) => { window.__fakeTranscript = t; }, transcript);
...
try {
  ...
  ok('AC1b. preview leggibile: titolo, paziente e testo consegna', /Aggiungi consegna/.test(previewTxt) && /Moretti/.test(previewTxt) && previewTxt.includes(CONSEGNA_TEXT));
  await shot('after-consegna-preview.png');
  await chatConfirm(page);
  const consegneAfter = await api('/consegne');
  const nuova = consegneAfter.find((c) => c.note === CONSEGNA_TEXT && !consegneBefore.some((b) => b.id === c.id));
  ok('AC1c. consegna salvata nel backend (stesso service della UI)', !!nuova, nuova?.id);
  ok('AC1d. consegna collegata al paziente giusto', nuova?.pazienteId === moretti?.id && /Moretti/.test(nuova?.pazienteNome ?? ''));
  ...
  ok('AC1e. consegna visibile nella UI consegne senza reload', (await page.locator(`text=${CONSEGNA_TEXT}`).count()) > 0);
  ...
  ok('AC1f. consegna persistente dopo refresh', (await page.locator(`text=${CONSEGNA_TEXT}`).count()) > 0);
  ...
  await page.waitForLoadState('networkidle');
  await openAgnos(page);
  const t2 = await dictate(page, 'Scrivi nel diario di Elena Moretti: paziente tranquillo, nessun dolore riferito');
  ...
  await page.waitForTimeout(1000);
  ok('AC2b. nota diario salvata e visibile nella sezione Diario', (await page.locator('text=paziente tranquillo, nessun dolore riferito').count()) > 0);
  ...
  ok('AC3b. parametro 120/80 salvato nella cartella (persistito)', vitali.some((v) => v.valore === '120/80'));
  await page.locator('.ai-drawer__scrim').click();
  await clickText(page, 'Panoramica');
  await page.waitForTimeout(800);
  ok('AC3c. parametro visibile nella sezione corretta senza reload', /120\/80/.test(await page.locator('body').innerText()));
  ...
  await openAgnos(page);
  const t4 = await dictate(page, 'crea appuntamento fisioterapia domani alle 09:30 per Elena Moretti');
  ...
  ok('AC4b. appuntamento creato e persistito in agenda', appts.some((a) => a.ora === '09:30' && a.patientId === moretti.id));
  await page.locator('.ai-drawer__scrim').click();
  ...
  ok('AC5. senza conferma esplicita non viene salvato nulla', consegneCountAfter === consegneCountBefore, `${consegneCountBefore} → ${consegneCountAfter}`);
  ...
  await page.click('.ai-asst__send');
  await page.waitForFunction((n) => document.querySelectorAll('.agnos-refusal').length > n, beforeVoiceRef, { timeout: 10000 });
  ...
  await page.waitForFunction((n) => document.querySelectorAll('.agnos-refusal').length > n, beforeChatRef, { timeout: 10000 });
  ...
  ok('AC6c. nessun dato cancellato dai tentativi', consegneFinal.some((c) => c.note === CONSEGNA_TEXT));

  // ── AC7 — Catalogo ispezionabile: ZERO azioni delete ────────────────────────
  const catalog = await api('/ai/actions/catalog', OP_HEADERS);
  ok('AC7a. catalogo: 0 azioni delete su ' + catalog.length, catalog.length > 0 && catalog.every((a) => a.kind !== 'delete'),
    catalog.map((a) => `${a.name}:${a.kind}`).join(', '));
  ok('AC7b. create_consegna presente nel catalogo (kind create)', catalog.some((a) => a.name === 'create_consegna' && a.kind === 'create'));
  ...
  ok('AC7c. audit: create_consegna eseguita con channel=voce', audit.some((e) => e.actionType === 'create_consegna' && e.channel === 'voce' && e.outcome === 'ok'));
  ok('AC7d. audit: rifiuti delete tracciati (refused_delete)', audit.some((e) => e.actionType === 'refused_delete'));
} finally {
  await ctx.close();
  await browser.close();
}
