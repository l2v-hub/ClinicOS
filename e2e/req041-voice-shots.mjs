...
const PATIENT = { id: 'p1', medicalRecordNumber: 'M1', firstName: 'Mario', lastName: 'Rossi', dateOfBirth: '1950-01-01', sex: 'M', phone: '0612345678', email: 'm.rossi@example.it', address: 'Via Vecchia 3' };

// /ai/voice/plan responses keyed by what the transcript contains.
function planFor(transcript) {
  const t = (transcript || '').toLowerCase();
  if (t.includes('per mario rossi') || t.includes('rossi')) return {
    plan: { actionType: 'create_vital_sign', patientId: null, idempotencyKey: 'idem-amb', sourceTranscript: transcript },
    preview: { actionType: 'create_vital_sign', patientId: null, title: 'Aggiungi parametro',
      lines: [{ label: 'Parametro', value: 'Pressione arteriosa' }, { label: 'Valore', value: '130/80 mmHg' }, { label: 'Orario', value: '09:00' }],
      ambiguities: ['Paziente non identificato con certezza'], canExecute: false, warnings: [] },
  };
  if (t.includes('pressione')) return {
    plan: { actionType: 'create_vital_sign', patientId: 'p1', idempotencyKey: 'idem-vital', sourceTranscript: transcript },
    preview: { actionType: 'create_vital_sign', patientId: 'p1', patientName: 'Rossi Mario', title: 'Aggiungi parametro',
      lines: [{ label: 'Parametro', value: 'Pressione arteriosa' }, { label: 'Valore', value: '130/80 mmHg' }, { label: 'Orario', value: '09:00' }],
      ambiguities: [], canExecute: true, warnings: [] },
  };
  if (t.includes('telefono')) return {
    plan: { actionType: 'update_patient_demographics', patientId: 'p1', idempotencyKey: 'idem-demo', sourceTranscript: transcript },
    preview: { actionType: 'update_patient_demographics', patientId: 'p1', patientName: 'Rossi Mario', title: 'Modifica dato anagrafico',
      lines: [{ label: 'Campo', value: 'Telefono' }, { label: 'Valore attuale', value: '0612345678' }, { label: 'Nuovo valore', value: '0698765432' }],
      ambiguities: [], canExecute: true, warnings: [] },
  };
  if (t.includes('anamnesi')) return {
    plan: { actionType: 'update_narrative_section', patientId: 'p1', idempotencyKey: 'idem-narr', sourceTranscript: transcript },
    preview: { actionType: 'update_narrative_section', patientId: 'p1', patientName: 'Rossi Mario', title: 'Aggiorna sezione narrativa',
      lines: [{ label: 'Sezione', value: 'ANAMNESIS' }],
      diff: { current: 'Ipertensione in trattamento.', proposed: 'paziente iperteso noto', resulting: 'Ipertensione in trattamento.\npaziente iperteso noto' },
      ambiguities: [], canExecute: true, warnings: [] },
  };
...
async function mock(p) {
  await p.route('**/*', async (r) => {
    ...
    if (u.includes('/ai/voice/stt')) return j({ available: false, model: null, degraded: true, reason: 'AI_STT_MODEL non configurato' });
    if (u.includes('/ai/voice/plan') && m === 'POST') { const body = JSON.parse(r.request().postData() || '{}'); return j(planFor(body.transcript)); }
    if (u.includes('/ai/voice/execute') && m === 'POST') return j({ ok: true, actionType: 'create_vital_sign', recordId: 'rec-1', message: 'Parametro registrato.', deduped: false });
...
try {
  ...
  await mock(p);
  await p.goto(FRONTEND, { waitUntil: 'networkidle', timeout: 30000 }); await p.waitForTimeout(500);
  await p.getByText('Operatore', { exact: true }).click(); await p.waitForTimeout(700);
  ...
  await p.getByRole('button', { name: 'Verifica comando' }).click();
  await p.waitForSelector('.voice-preview', { timeout: 5000 }); await p.waitForTimeout(250);
  await shot(p, 'voice-vital-sign-preview.png');

  // 4) confirmed
  await p.getByRole('button', { name: 'Conferma e salva' }).click();
  await p.waitForSelector('.voice-done', { timeout: 5000 }); await p.waitForTimeout(200);
  ...
  await mock(p2);
  await p2.goto(FRONTEND, { waitUntil: 'networkidle', timeout: 30000 }); await p2.waitForTimeout(500);
  await p2.getByText('Operatore', { exact: true }).click(); await p2.waitForTimeout(500);
  await openVoice(p2);
  await p2.getByRole('button', { name: /Parla/ }).click();
  await p2.waitForSelector('.voice-state--permesso-negato', { timeout: 5000 }).catch(() => {});
  await p2.waitForTimeout(200);
  await p2.locator('.voice-drawer').screenshot({ path: resolve(out, 'voice-permission-denied.png') });
  ...
  const ctxT = await browser.newContext({ viewport: { width: 1024, height: 768 }, permissions: ['microphone'] });
  const pt = await ctxT.newPage(); await mock(pt);
  await pt.goto(FRONTEND, { waitUntil: 'networkidle', timeout: 30000 }); await pt.waitForTimeout(500);
  await pt.getByText('Operatore', { exact: true }).click(); await pt.waitForTimeout(500);
  await openVoice(pt);
  ...
  await pt.waitForSelector('.voice-preview', { timeout: 5000 }); await pt.waitForTimeout(250);
  await pt.locator('.voice-drawer').screenshot({ path: resolve(out, 'voice-vital-sign-preview-tablet.png') });
