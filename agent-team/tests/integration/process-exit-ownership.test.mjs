...
test('host termination takes down the owned process tree (QA-263-015)', async () => {
  ...
  const host = spawn(process.execPath, [fixture, pidFile], { cwd: path.resolve('.'), stdio: 'ignore', windowsHide: true });
  ...
  try {
    ...
    assert.ok(Number.isInteger(grandchildPid), 'the owned grandchild must have started and reported its pid');

    await hostExit;

    let alive = true;
    for (let waited = 0; waited < 8000 && alive; waited += 200) {
      try { process.kill(grandchildPid, 0); await delay(200); } catch { alive = false; }
    }
    assert.equal(alive, false, `host termination must kill the owned process tree; pid ${grandchildPid} survived`);
  } finally {
    // Never leave a stray process behind, even when the assertion above fails (RED phase).
    if (Number.isInteger(grandchildPid)) { try { process.kill(grandchildPid); } catch { /* already dead */ } }
    try { host.kill(); } catch { /* already exited */ }
  }
});
