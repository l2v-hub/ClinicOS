...
const blockedSchema = JSON.parse(await readFile(path.resolve('agent-team/protocol/schemas/worker-blocked.schema.json'), 'utf8'));

const identity = (attempt, extra = {}) => ({ schema_version: 1, message_id: `m-${attempt}-${extra.kind}`, correlation_id: `issue-263-attempt-${attempt}`, repository: 'l2v-hub/ClinicOS', issue_number: 263, pull_request_number: 264, attempt, created_at: `2026-07-12T1${attempt}:00:00Z`, state_before: 'x', state_after: 'y' });
const handoffMsg = (attempt, sha) => ({ ...identity(attempt, { kind: 'handoff' }), message_type: 'development_handoff', producer_role: 'claude-development', subject_sha: sha, acceptance_criteria: [], resolved_findings: [], commands: [], artifact_refs: [], next_actions: ['ready-for-qa'] });
const qaFailedMsg = (attempt, sha, fingerprint) => ({ ...identity(attempt, { kind: 'qa' }), message_type: 'qa_result', producer_role: 'codex-qa', subject_sha: sha, decision: 'qa-failed', acceptance_criteria: [], findings: [{ finding_id: 'QA-263-001', acceptance_criterion_id: 'AC8', severity: 'critical', category: 'remediation-loop', observed: 'findings never reach Claude', expected: 'findings reach Claude', reproduction_command_ref: 'rg', evidence_refs: ['runtime.mjs'], status: 'open', remediation_required: true, fingerprint }], commands: [], artifact_refs: [], next_actions: ['ready-for-dev', 'assigned-to-claude'] });
...
test('three equivalent qa-failed results block the issue with a structured reason and no Claude launch', async () => {
  ...
  const github = githubFake([handoffMsg(1, S1), qaFailedMsg(1, S1, fp), qaFailedMsg(2, S1, fp), qaFailedMsg(3, S1, fp)]);
  const run = async (call) => { throw new Error(`no process may run, got: ${call.command}`); };
  const git = { async prepareIssueWorktree() { throw new Error('worktree must not be prepared'); }, async headSha() { return S1; } };
  const runtime = await createRuntime({ config, repoRoot, overrides: { run, github, git, workerId: 'host:test', doctor: async () => ({ ok: false, developmentReady: true, qaReady: false, checks: [] }) } });
  ...
  const blockedMessages = github.state.comments.map((c) => { try { return parseProtocolComment(c.body, blockedSchema); } catch { return null; } }).filter((v) => v?.message_type === 'worker.blocked');
