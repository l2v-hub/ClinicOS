    ...
    items: [
      {
        issue: {
          ...
          comments: [comment(1, claim), comment(2, handoff), comment(3, qaResult)],
        },
        pullRequest: {
          number: 264,
          isDraft: true,
...
test('status projection reports no active claim once a matching release exists', () => {
  ...
  const release = {
    ...
    message_id: 'claim-263-2-abc-released',
    released_at: '2026-07-13T08:30:00Z',
    release_reason: 'development-handoff-published: dev-263-2-handoff-abc',
  };
  const projection = buildStatusProjection({
    ...
    items: [
      {
        issue: {
          ...
          comments: [comment(1, invalidClaim), comment(2, release)],
        },
        pullRequest: {
          number: 264,
          isDraft: true,
  ...
  assert.equal(
    issue.active_claim,
    null,
    'a released claim must not be projected as active before lease expiry',
  );
...
test('restart recovery refreshes my unexpired claims from GitHub state', async () => {
  ...
  const github = {
    async listIssuesByLabels(labels) {
      return labels.includes('agent-working')
        ? [{ number: 263, labels: [{ name: 'agent-working' }] }]
        : [];
    },
    async listIssueComments() {
      return [comment(7, claim)];
    },
