...
const qaFailedMsg = (attempt, sha) => ({
  ...
  findings: [
    {
      finding_id: 'QA-263-013',
      acceptance_criterion_id: 'AC11',
      severity: 'high',
      category: 'recovery-worktree',
      observed: 'once hides worker failures',
      expected: 'once reports failure',
      reproduction_command_ref: 'npm run agent-team:once',
      evidence_refs: ['once.mjs'],
      status: 'open',
...
test('reconciliationOutcome marks a pass with worker errors as failed even when doctor is ready', () => {
  ...
  assert.equal(
    reconciliationOutcome({
      doctor: readyDoctor,
      development: { errors: [{ number: 263, error: 'spawn failed' }], processed: [263] },
      qa: { errors: [], processed: [] },
    }).ok,
    false,
    'a development worker failure must fail the pass',
  );
  assert.equal(
    reconciliationOutcome({
      doctor: readyDoctor,
      development: { errors: [], processed: [] },
      qa: { errors: [{ number: 263, error: 'qa crashed' }], processed: [263] },
    }).ok,
    false,
    'a qa worker failure must fail the pass',
  );
  assert.equal(
    reconciliationOutcome({
      doctor: { developmentReady: false, qaReady: false },
      development: { errors: [], processed: [] },
      qa: { errors: [], processed: [] },
    }).ok,
    false,
    'a doctor-hard-failed pass stays failed',
  );
});
...
test('once returns ok:false when the development worker fails, so the CLI exits non-zero (QA-263-013)', async () => {
  ...
  const run = async (call) => {
    if (call.command === 'claude')
      return {
        ...
        stdout: '',
        stderr: '',
        error: 'working directory does not exist: agent-team/.worktrees/issue-263',
      };
  ...
  const result = await runOnce({
    config,
    repoRoot,
    allowCurrentSupervisor: true,
    overrides: {
      run,
      github,
      git,
      workerId: 'host:test',
      doctor: async () => ({ ok: true, developmentReady: true, qaReady: false, checks: [] }),
    },
  });
  assert.equal(
    result.development.errors.length,
    1,
    'the worker failure must surface in development.errors',
  );
  assert.match(result.development.errors[0].error, /working directory does not exist/);
  assert.equal(
    result.ok,
    false,
    'once must report the failed pass unambiguously (ok:false -> exit 1)',
  );
});
