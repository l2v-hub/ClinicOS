...
async function assertDead(pid) {
  ...
  for (let attempt = 0; attempt < 50 && alive; attempt += 1) {
    try { process.kill(pid, 0); await delay(100); } catch { alive = false; }
  }
...
test('runProcess abort signal kills the owned process tree with an explicit abort diagnostic (QA-263-015)', async () => {
  ...
  const promise = runner.runProcess({
    command: process.execPath, args: ['-e', 'setTimeout(() => {}, 3000)'], cwd: process.cwd(),
    timeoutMs: 30000, maxOutputBytes: 1024, signal: controller.signal
  });
...
test('an already-aborted signal refuses to spawn at all (QA-263-015)', async () => {
  ...
  const result = await runner.runProcess({
    ...
    timeoutMs: 5000, maxOutputBytes: 1024, signal: controller.signal
  });
...
test('killOwnedProcessTrees terminates every live owned child so supervisor termination leaves no orphan (QA-263-015)', async () => {
  assert.equal(typeof runner.killOwnedProcessTrees, 'function', 'the runner must expose killOwnedProcessTrees for supervisor shutdown');
  const promise = runner.runProcess({
    command: process.execPath, args: ['-e', 'setTimeout(() => {}, 8000)'], cwd: process.cwd(),
...
test('finished children leave the owned registry (QA-263-015)', async () => {
  ...
  assert.equal(runner.killOwnedProcessTrees(), 0, 'a completed child must not linger in the owned-process registry');
});
