...
test('config requires a non-empty scoped allowedTools policy without wildcards or bypass flags', () => {
  ...
  assert.throws(
    () => validateConfig({ ...baseConfig, allowedTools: [] }),
    /allowedTools must be a non-empty array/,
  );
  assert.throws(
    () => validateConfig({ ...baseConfig, allowedTools: ['*'] }),
    /allowedTools entry must be a scoped tool rule/,
  );
  assert.throws(
    () =>
      validateConfig({
        ...baseConfig,
        allowedTools: ['Bash(claude --dangerously-skip-permissions *)'],
      }),
    /allowedTools entry must not reference bypass options/,
  );
});
...
test('config requires an explicit tool surface and nested-execution denies (QA-263-011)', () => {
  ...
  assert.throws(
    () => validateConfig({ ...baseConfig, tools: [...SAFE_TOOLS, 'Task'] }),
    /tools must not expose a nested-agent tool: Task/,
  );
  assert.throws(
    () => validateConfig({ ...baseConfig, tools: [...SAFE_TOOLS, 'Agent'] }),
    /tools must not expose a nested-agent tool: Agent/,
  );
  assert.throws(
    () =>
      validateConfig({
        ...baseConfig,
        disallowedTools: SAFE_DISALLOWED.filter((rule) => rule !== 'Bash(claude *)'),
      }),
    /disallowedTools must deny nested Claude execution: missing Bash\(claude \*\)/,
  );
});

test('config requires realistic development and QA timeouts above the command timeout', () => {
  assert.throws(
    () => validateConfig({ ...baseConfig, developmentTimeoutMs: 120000 }),
    /developmentTimeoutMs must exceed commandTimeoutMs/,
  );
  assert.throws(
    () => validateConfig({ ...baseConfig, qaTimeoutMs: 60000 }),
    /qaTimeoutMs must exceed commandTimeoutMs/,
  );
});
...
test('Claude worker passes the scoped allowedTools policy and the development timeout', async () => {
  ...
  assert.equal(
    argv.some((arg) => arg.includes('dangerously')),
    false,
  );
});
  ...
  await assert.rejects(
    () =>
      runClaudeDevelopment({
        issue: { number: 263, title: 't', body: 'b' },
        attempt: 2,
        config: { ...baseConfig, allowedTools: [] },
        github: githubFake,
        git: worktree,
        run,
        schema: { type: 'object' },
...
test('Claude worker rejects a structured handoff that violates the required schema before any GitHub change', async () => {
  ...
  const github = {
    ...
    addPullRequestComment: async () => {
      githubCalls.push('pr-comment');
    },
    addLabels: async () => {
      githubCalls.push('add-labels');
    },
    removeLabels: async () => {
      githubCalls.push('remove-labels');
    },
  };
  const run = async () => ({
    ok: true,
    code: 0,
    stdout: JSON.stringify({ structured_output: { unexpected: true } }),
    stderr: '',
    error: null,
  });
  await assert.rejects(
    () =>
      runClaudeDevelopment({
        ...
        git: worktree,
        run,
        schema: { type: 'object', required: ['message_type', 'subject_sha'] },
        priorQaResult: null,
      }),
...
test('Codex QA worker uses the dedicated QA timeout', async () => {
  ...
  const run = async (call) => {
    ...
    return {
      ...
      stderr: '',
      error: null,
      finalMessage: JSON.stringify(qa),
    };
  };
  await runCodexQa({
    issue: { number: 263 },
    pullRequest: { number: 264, headRefOid: 'a'.repeat(40) },
    config: { ...baseConfig, runtimeRoot: (await import('node:os')).tmpdir() },
    github: githubFake,
    run,
    schema: { type: 'object' },
    worktreePath: 'C:/tmp/issue-263',
  });
