...
const stepClinicaPath = path.join(
  repoRoot,
  'frontend',
  'src',
  'components',
...
test('browser journey completes every required intake confirmation (QA-263-014)', async () => {
  ...
  assert.match(
    script,
    /accept-therapy/,
    'the journey must check the #235 therapy acceptance in step 3 (Clinica)',
  );
  assert.match(
    script,
    /accept-demographics/,
    'the journey must check the #235 demographics acceptance in step 6 (Verifica)',
  );
  assert.match(
    script,
    /intake-step-3/,
    'the journey must wait for the IntakeWorkspace handoff (step 3 Clinica)',
  );
  assert.match(
    script,
    /intake-step-6/,
    'the journey must reach step 6 (Verifica) before creating the patient',
  );
  assert.match(
    script,
    /allergy-status-/,
    'the journey must document the allergy state instead of leaving it "non documentato"',
  );
});
...
test('browser journey asserts a persisted patient, not transient form text (QA-263-014)', async () => {
  ...
  assert.match(
    script,
    /\/patients/,
    'the journey must verify creation against the backend /patients API',
  );
  assert.match(
    script,
    /reload|goto\(FRONTEND/,
    'the journey must re-load the SPA and verify the patient survives a reload',
  );
  assert.match(
    script,
    /process\.exit\(failures \? 1 : 0\)/,
    'a non-created outcome must remain a hard failure (regression safety)',
  );
