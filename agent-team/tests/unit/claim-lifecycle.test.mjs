...
function githubFake() {
  ...
  return {
    ...
    async editIssueComment(commentId, body) {
      state.edits.push(commentId);
...
test('acquireGitHubClaim publishes a schema-valid claim and ignores invalid competitors', async () => {
  ...
  const result = await acquireGitHubClaim({
    github,
    config,
    schema: claimSchema,
    issue: { number: 263 },
...
test('releaseGitHubClaim publishes a schema-valid work.claim_released message', async () => {
  ...
  await releaseGitHubClaim({
    ...
    now: new Date('2026-07-13T08:05:00Z'),
  });
...
test('a released claim no longer competes in acquire arbitration', async () => {
  const github = githubFake();
  const first = await acquireGitHubClaim({
    github,
    config,
    schema: claimSchema,
    issue: { number: 263 },
  ...
  await releaseGitHubClaim({
    ...
    schema: claimSchema,
    claim: first.claim,
    reason: 'development-handoff-published: dev-263-2-handoff-abc',
    now: new Date('2026-07-13T08:01:00Z'),
  });
  const second = await acquireGitHubClaim({
    github,
    config,
    schema: claimSchema,
    issue: { number: 263 },
    ...
    role: 'claude-development',
    attempt: 2,
    now: new Date('2026-07-13T08:02:00Z'),
  });
  assert.equal(
    second.won,
    true,
    'a released but unexpired claim must not defeat the next live claim',
  );
});
...
test('recoverActiveClaim ignores my claim once a matching release exists', async () => {
  ...
  const active = recoverActiveClaim({
    comments: github.state.comments,
    schema: claimSchema,
    issueNumber: 263,
    workerId: 'host:1',
    now: new Date('2026-07-13T08:02:00Z'),
...
test('recoverActiveClaim returns my unexpired claim and null when expired', () => {
  ...
  const comments = [
    { id: 5, created_at: '2026-07-13T08:00:00Z', body: `${MARKER}\n${JSON.stringify(mine)}` },
  ];
  const active = recoverActiveClaim({
    comments,
    schema: claimSchema,
    issueNumber: 263,
    workerId: 'host:1',
    now: new Date('2026-07-13T08:30:00Z'),
  });
