...
const comment = (id, value) => ({
  id,
  created_at: value.created_at ?? '2026-07-12T14:00:00Z',
...
const handoff1 = {
  ...
  message_id: 'dev-263-1',
  correlation_id: 'issue-263-attempt-1',
  producer_role: 'claude-development',
  repository: 'l2v-hub/ClinicOS',
  issue_number: 263,
  pull_request_number: 264,
  attempt: 1,
  created_at: '2026-07-12T14:41:05Z',
  subject_sha: '6'.repeat(40),
  state_before: 'agent-working',
  state_after: 'ready-for-qa',
  acceptance_criteria: [],
  resolved_findings: [],
  commands: [],
  artifact_refs: [],
...
const qaFail1 = {
  ...
  pull_request_number: 264,
  attempt: 1,
  created_at: '2026-07-12T18:20:43Z',
  subject_sha: '6'.repeat(40),
  state_before: 'ready-for-qa',
  state_after: 'qa-failed',
  decision: 'qa-failed',
  acceptance_criteria: [],
  findings: [
    {
    ...
    {
      finding_id: 'QA-263-002',
      acceptance_criterion_id: 'AC5',
      severity: 'critical',
...
test('rebuildRemediationContext returns next attempt, open findings, and stable coordinates', () => {
  ...
  assert.equal(
    context.priorQaResult.coordinates.worktree_path,
    path.join('C:/repo/agent-team/.worktrees', 'issue-263'),
  );
...
test('rebuildRemediationContext reports blocked after three equivalent failed attempts', () => {
  const equivalent = (attempt, commentId) =>
    comment(commentId, {
      ...qaFail1,
      attempt,
      message_id: `qa-263-${attempt}`,
