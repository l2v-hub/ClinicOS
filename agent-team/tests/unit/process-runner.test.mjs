...
test('sanitizeText removes credentials and authorization values', () => {
  const value = sanitizeText(
    'Authorization: Bearer secret-token OPENAI_API_KEY=sk-live patientId=12345',
  );
...
test('runProcess reports a missing executable without throwing', async () => {
  const result = await runProcess({
    command: 'definitely-missing-clinicos-cli',
    args: [],
    cwd: process.cwd(),
    timeoutMs: 1000,
    maxOutputBytes: 1024,
...
test('runProcess refuses a nonexistent working directory with an unambiguous diagnostic instead of a misleading spawn ENOENT (QA-263-013)', async () => {
  ...
  assert.match(
    ...
    'the error must name the invalid cwd condition, not the executable',
  );
  assert.doesNotMatch(
    result.error,
    /spawn node ENOENT/,
    'an invalid cwd must not masquerade as a missing executable',
  );
});
...
test('runProcess default timeout kill actually terminates the spawned process', async () => {
  ...
  assert.equal(
    ...
    true,
    'the result must expose the child pid for liveness verification',
  );
  let alive = true;
