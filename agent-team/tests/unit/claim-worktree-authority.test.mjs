...
function fakeGitRun(handlers) {
  ...
  const run = async ({ command, args, cwd }) => {
    ...
    for (const [pattern, responder] of handlers) {
      if (pattern.test(key))
        return typeof responder === 'function' ? responder({ args, cwd }) : responder;
    }
    return {
      ok: false,
      code: 1,
      stdout: '',
      stderr: `unhandled git invocation: ${key}`,
      error: null,
    };
  };
  return { run, calls };
...
const worktreeList = (entries) =>
  entries
    .map(
      ({ dir, branch }) => `worktree ${dir}\nHEAD ${'0'.repeat(40)}\nbranch refs/heads/${branch}\n`,
    )
    .join('\n');
...
test('a nested subdirectory of a registered worktree is never adopted as claim coordinates (QA-263-015)', async () => {
  ...
  const coordinates = await git.prepareIssueWorktree({
    issue: { number: 263, title: 'Agent Team' },
    prior: { worktree_path: nested, branch: BRANCH, pull_request_number: 264 },
  });
  assert.equal(
    coordinates.path,
    registeredRoot,
    'an unregistered nested path must be repaired to the registered worktree root of the same branch',
  );
});
...
test('resolveIssueWorktree resolves the authoritative registered checkout without any mutating git command (QA-263-015)', async () => {
  ...
  const staleRecorded = path.join(
    tmpdir(),
    `agent-team-resolve-missing-${Date.now()}`,
    'issue-263',
  );
  ...
  const git = createGitAdapter({ run, config: adapterConfig });
  assert.equal(
    typeof git.resolveIssueWorktree,
    'function',
    'the git adapter must expose a read-only resolveIssueWorktree',
  );
  const resolved = await git.resolveIssueWorktree({
    issue: { number: 263, title: 'Agent Team' },
    prior: { worktree_path: staleRecorded, branch: BRANCH, pull_request_number: 264 },
  });
  assert.equal(
    resolved.path,
    registeredRoot,
    'resolution must land on the registered checkout of the branch',
  );
  assert.equal(resolved.branch, BRANCH);
  for (const { args } of calls) {
    const key = args.join(' ');
    for (const pattern of MUTATING)
      assert.equal(pattern.test(key), false, `claim resolution must be read-only, got: git ${key}`);
  }
});
  ...
  findings: [
    {
      ...
      observed: 'claim named a non-authoritative nested path',
      expected: 'claim cwd equals a registered worktree',
      reproduction_command_ref: 'git worktree list --porcelain',
...
test('the published claim names the exact authoritative worktree the Claude process runs in (QA-263-015)', async () => {
  ...
  const git = {
    async resolveIssueWorktree() {
      return { path: authoritative, branch: BRANCH, pullRequestNumber: 264 };
    },
  ...
  assert.equal(
    claims[0].worktree,
    authoritative,
    'the claim must name the authoritative registered worktree, not the recorded nested default',
  );
  assert.equal(claudeCalls.length, 1);
  assert.equal(
    claudeCalls[0].cwd,
    authoritative,
    'the Claude process must run in the claimed worktree',
  );
  assert.equal(
    claims[0].worktree,
    claudeCalls[0].cwd,
    'claim coordinates and process cwd must be identical',
  );
});
...
test('a prepared worktree that differs from the claimed coordinates fails closed and restores the claimable state (QA-263-015)', async () => {
  ...
  const github = githubFake([handoffMsg(1, S1), qaFailedMsg(1, S1)]);
  const run = async (call) => {
    throw new Error(`no process may run when coordinates diverge, got: ${call.command}`);
  };
  const git = {
    async resolveIssueWorktree() {
      return { path: claimed, branch: BRANCH, pullRequestNumber: 264 };
    },
    async prepareIssueWorktree() {
      return { path: elsewhere, branch: BRANCH, pullRequestNumber: 264 };
    },
    async headSha() {
      return S2;
    },
  ...
  assert.equal(
    ...
    1,
    'diverging coordinates must surface as a worker failure',
  );
