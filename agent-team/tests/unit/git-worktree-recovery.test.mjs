  ...
  for (const { args } of calls) {
    ...
    for (const pattern of DESTRUCTIVE)
      assert.equal(
        pattern.test(key),
        false,
        `destructive git invocation is forbidden during recovery: git ${key}`,
      );
  }
};
...
test('a recorded worktree path that no longer exists is never blindly adopted: the existing checkout of the same branch is reused (QA-263-013)', async () => {
  ...
  const coordinates = await git.prepareIssueWorktree({
    ...
    prior: prior(staleRecorded),
  });
  assert.equal(
    coordinates.path,
    existingCheckout,
    'stale recorded coordinates must be repaired to the live checkout of the expected branch',
  );
...
test('stale coordinates with no live checkout are recreated from the existing branch without destructive commands (QA-263-013)', async () => {
  const staleRecorded = path.join(
    tmpdir(),
    `agent-team-missing-${Date.now()}-recreate`,
    'issue-263',
  );
  const { run, calls } = fakeRun([
  ...
  assert.equal(
    coordinates.path,
    staleRecorded,
    'the recreated worktree must live at the recorded coordinates',
  );
  const keys = calls.map(({ args }) => args.join(' '));
  assert.ok(
    keys.some((key) => key === `worktree add ${staleRecorded} ${BRANCH}`),
    `the existing branch must be checked out again, got: ${keys.join(' | ')}`,
  );
  assert.ok(
    keys.includes('worktree prune'),
    'stale worktree registrations must be pruned before recreation',
  );
  assertNoDestructiveGit(calls);
});
...
test('valid recorded coordinates (existing directory, expected branch and repository) are reused as-is', async () => {
  ...
  const coordinates = await git.prepareIssueWorktree({
    issue: { number: 263, title: 'Agent Team' },
    prior: prior(existing),
  });
  assert.equal(coordinates.path, existing);
  const keys = calls.map(({ args }) => args.join(' '));
  assert.equal(
    keys.some((key) => key.startsWith('worktree add') || key.startsWith('worktree prune')),
    false,
    'a valid worktree must not be recreated',
  );
});
...
test('an existing directory on an unexpected branch or foreign repository is not reused and nothing is destroyed', async () => {
  ...
  const { run, calls } = fakeRun([
    ...
    [
      ...
      ok(worktreeList([{ dir: wrongBranchDir, branch: 'some-other-branch' }])),
    ],
    [/^worktree prune$/, ok()],
    [/^fetch origin$/, ok()],
    [
      /^rev-parse --verify --quiet refs\/heads\//,
      { ok: false, code: 1, stdout: '', stderr: '', error: null },
    ],
    [
      /^worktree add /,
      {
        ...
        code: 128,
        stdout: '',
        stderr: `fatal: '${wrongBranchDir}' already exists`,
        error: null,
      },
    ],
  ]);
  ...
  await assert.rejects(
    () =>
      git.prepareIssueWorktree({
        issue: { number: 263, title: 'Agent Team' },
        prior: prior(wrongBranchDir),
      }),
...
test('a worktree belonging to a foreign repository is repaired to a checkout of the expected repository', async () => {
  ...
  const { run, calls } = fakeRun([
    ...
    [
      ...
      ({ cwd }) =>
        ok(cwd === foreignDir ? 'https://github.com/someone-else/other-repo.git' : ORIGIN),
    ],
  ...
  const coordinates = await git.prepareIssueWorktree({
    issue: { number: 263, title: 'Agent Team' },
    prior: prior(foreignDir),
  });
  assert.equal(
    coordinates.path,
    rightDir,
    'coordinates in a foreign repository must never be adopted',
  );
  assertNoDestructiveGit(calls);
});
