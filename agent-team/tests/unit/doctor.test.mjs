...
const config = {
  ...
  runtimeRoot: 'C:/repo/agent-team/.runtime',
  worktreeRoot: 'C:/repo/agent-team/.worktrees',
...
const CLAUDE_HELP =
  'Usage: claude [options] --print --output-format <fmt> --permission-mode <mode> --tools <tools...> --allowedTools <tools> --disallowedTools <tools...>';
const CODEX_EXEC_HELP =
  'Usage: codex exec [options] --sandbox <mode> --cd <dir> --output-schema <file> --output-last-message <file>';
...
function healthyRun(overrides = {}) {
  return async ({ command, args }) => {
    ...
    if (key.startsWith('gh label list'))
      return result(JSON.stringify(Object.values(config.labels).map((name) => ({ name }))));
...
const doctorArgs = (extra = {}) => ({
  config,
  run: healthyRun(extra.overrides),
  isSupervisorLive: extra.isSupervisorLive ?? (async () => false),
  probes: extra.probes ?? probes,
});
...
test('doctor passes every contract prerequisite on a healthy workstation', async () => {
  ...
  for (const id of [
    ...
    assert.equal(
      doctor.checks.find((check) => check.id === id)?.ok,
      true,
      `check ${id} must exist and pass`,
    );
  }
...
test('doctor marks development unavailable when Claude is unauthenticated', async () => {
  const doctor = await runDoctor(
    doctorArgs({ overrides: { 'claude auth status': result('{"loggedIn":false}', 1) } }),
  );
...
test('doctor refuses a duplicate local supervisor and passes the configured heartbeat timeout', async () => {
  ...
  const doctor = await runDoctor(
    doctorArgs({
      isSupervisorLive: async (options) => {
        received = options;
        return true;
      },
    }),
...
test('doctor fails closed when a required GitHub label is missing', async () => {
  const partial = Object.values(config.labels)
    .filter((name) => name !== 'agent-working')
    .map((name) => ({ name }));
  const doctor = await runDoctor(
    doctorArgs({
      overrides: {
        'gh label list --repo l2v-hub/ClinicOS --limit 200 --json name': result(
...
test('doctor fails closed without push permission on the repository', async () => {
  const doctor = await runDoctor(
    doctorArgs({
      overrides: {
        'gh api repos/l2v-hub/ClinicOS': result(JSON.stringify({ permissions: { push: false } })),
      },
    }),
  );
...
test('doctor blocks development when claude help lacks a required worker option', async () => {
  const doctor = await runDoctor(
    doctorArgs({
      overrides: {
        'claude --help': result('Usage: claude --print --output-format --permission-mode'),
      },
    }),
  );
...
test('doctor blocks QA when codex exec help lacks a required option', async () => {
  const doctor = await runDoctor(
    doctorArgs({
      overrides: {
        'codex exec --help': result('Usage: codex exec --sandbox --cd --output-last-message'),
      },
    }),
  );
...
test('doctor fails closed when referenced prompt or schema files are missing', async () => {
  const doctor = await runDoctor(
    doctorArgs({ probes: { ...probes, fileExists: async () => false } }),
  );
...
test('doctor blocks development when claude help lacks --tools or --disallowedTools (QA-263-011)', async () => {
  const doctor = await runDoctor(
    doctorArgs({
      overrides: {
        'claude --help': result(
          'Usage: claude --print --output-format <fmt> --permission-mode <mode> --allowedTools <tools>',
        ),
      },
    }),
  );
...
test('doctor blocks development when the tool surface exposes a nested-agent tool (QA-263-011)', async () => {
  const doctor = await runDoctor({
    ...doctorArgs(),
    config: { ...config, tools: [...config.tools, 'Task'] },
  });
...
test('doctor blocks development when a required nested-execution deny rule is missing (QA-263-011)', async () => {
  const doctor = await runDoctor({
    ...
    config: {
      ...config,
      disallowedTools: config.disallowedTools.filter((rule) => rule !== 'Agent'),
    },
  });
  const check = doctor.checks.find((c) => c.id === 'worker-permission-policy');
  assert.equal(check.ok, false);
...
test('doctor fails closed on unrealistic worker timeouts', async () => {
  const doctor = await runDoctor({
    ...doctorArgs(),
    config: { ...config, developmentTimeoutMs: 60000 },
  });
...
test('doctor fails closed when the runtime root is not writable', async () => {
  const doctor = await runDoctor(
    doctorArgs({ probes: { ...probes, writableProbe: async () => false } }),
  );
