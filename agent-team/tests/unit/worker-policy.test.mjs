...
import { validateWorkerToolPolicy, buildClaudeWorkerArgs, NESTED_AGENT_TOOLS, REQUIRED_DISALLOWED_TOOLS } from '../../src/core/worker-policy.mjs';
...
const SAFE_ALLOWED = ['Read', 'Edit', 'Write', 'Glob', 'Grep', 'Bash(git *)', 'Bash(gh *)', 'Bash(node *)', 'Bash(npm run *)', 'Bash(npx *)', 'Bash(mkdir *)'];
const safeConfig = { tools: SAFE_TOOLS, allowedTools: SAFE_ALLOWED, disallowedTools: SAFE_DISALLOWED };
...
test('buildClaudeWorkerArgs produces the exact safe argv with --tools and --disallowedTools', () => {
  assert.deepEqual(buildClaudeWorkerArgs(safeConfig), [
    ...
    '--tools', SAFE_TOOLS.join(','),
    '--allowedTools', SAFE_ALLOWED.join(','),
    '--disallowedTools', SAFE_DISALLOWED.join(',')
  ]);
});

test('the built argv never references bypass or dangerous permission modes', () => {
  const argv = buildClaudeWorkerArgs(safeConfig);
  assert.equal(argv.some((arg) => /bypass|dangerous/i.test(arg)), false);
...
test('a policy exposing a nested-agent tool is rejected with a precise diagnostic', () => {
  for (const nested of ['Task', 'Agent']) {
    ...
    assert.match(policy.errors.join('; '), new RegExp(`tools must not expose a nested-agent tool: ${nested}`));
  }
});

test('a policy missing a required nested-execution deny rule is rejected', () => {
  for (const required of SAFE_DISALLOWED) {
    const policy = validateWorkerToolPolicy({ ...safeConfig, disallowedTools: SAFE_DISALLOWED.filter((rule) => rule !== required) });
    assert.equal(policy.ok, false, `dropping ${required} must invalidate the policy`);
    assert.match(policy.errors.join('; '), /disallowedTools must deny nested Claude execution: missing /);
  }
});
...
test('empty or missing tool policies fail closed', () => {
  ...
  assert.match(validateWorkerToolPolicy({ ...safeConfig, disallowedTools: undefined }).errors.join('; '), /disallowedTools policy is empty or missing/);
});

test('bypass tokens are rejected in every policy list', () => {
  assert.equal(validateWorkerToolPolicy({ ...safeConfig, allowedTools: [...SAFE_ALLOWED, 'Bash(claude --dangerously-skip-permissions *)'] }).ok, false);
  assert.equal(validateWorkerToolPolicy({ ...safeConfig, tools: [...SAFE_TOOLS, 'BypassTool'] }).ok, false);
});

test('buildClaudeWorkerArgs refuses an unsafe policy before any spawn', () => {
  assert.throws(() => buildClaudeWorkerArgs({ ...safeConfig, tools: [...SAFE_TOOLS, 'Task'] }), /claude development launch refused: .*nested-agent/);
  assert.throws(() => buildClaudeWorkerArgs({ ...safeConfig, disallowedTools: [] }), /claude development launch refused: .*disallowedTools/);
});
