...
test('startLeaseHeartbeat refreshes the lease on the configured interval and stop() halts it (QA-263-015)', async () => {
  assert.equal(typeof locks.startLeaseHeartbeat, 'function', 'locks must expose startLeaseHeartbeat');
  let refreshes = 0;
  const heartbeat = locks.startLeaseHeartbeat({ refresh: async () => { refreshes += 1; }, intervalMs: 20 });
  await delay(110);
  heartbeat.stop();
...
test('startLeaseHeartbeat aborts its signal when the lease refresh fails (QA-263-015)', async () => {
  const heartbeat = locks.startLeaseHeartbeat({ refresh: async () => { throw new Error('comment not found'); }, intervalMs: 15 });
  try {
    for (let waited = 0; waited < 2000 && !heartbeat.signal.aborted; waited += 20) await delay(20);
    assert.equal(heartbeat.signal.aborted, true, 'a failed refresh means the lease is lost and the signal must abort');
    assert.match(String(heartbeat.signal.reason?.message ?? heartbeat.signal.reason), /lease/i, 'the abort reason must name the lost lease');
  } finally { heartbeat.stop(); }
});

const workerConfig = {
  repository: 'l2v-hub/ClinicOS', baseBranch: 'origin/main', commandTimeoutMs: 1000, developmentTimeoutMs: 5400000,
  ...
  maxOutputBytes: 10000, labels: { working: 'agent-working', readyForQa: 'ready-for-qa' }
};
...
test('the Claude worker refreshes its lease while the process runs and hands the heartbeat signal to the runner (QA-263-015)', async () => {
  let refreshes = 0;
  const calls = [];
  const handoff = { schema_version: 1, message_type: 'development_handoff', issue_number: 263, pull_request_number: 300, attempt: 2, subject_sha: 'a'.repeat(40), acceptance_criteria: [], resolved_findings: [], commands: [], artifact_refs: [], next_actions: ['ready-for-qa'] };
  const run = async (call) => { calls.push(call); await delay(120); return { ok: true, code: 0, stdout: JSON.stringify({ structured_output: handoff }), stderr: '', error: null }; };
  ...
  await runClaudeDevelopment({
    issue: { number: 263, title: 'Agent Team', body: 'contract' }, attempt: 2, config: workerConfig, github, git, run,
    schema: { type: 'object' }, priorQaResult: null,
    lease: { refresh: async () => { refreshes += 1; }, intervalMs: 20 }
  });
  assert.ok(refreshes >= 2, `the lease must be refreshed during Claude execution, got ${refreshes} refreshes`);
  assert.ok(calls[0].signal instanceof AbortSignal, 'the runner must receive the heartbeat abort signal so a lost lease can kill the owned process tree');
});
...
test('a lease lost during Claude execution aborts the owned process and restores the claimable state (QA-263-015)', async () => {
  ...
  const state = {
    ...
    comments: [handoffMsg(1, S1), qaFailedMsg(1, S1)].map((value, index) => ({ id: index + 1, created_at: value.created_at, body: `${MARKER}\n${JSON.stringify(value)}` })),
    nextId: 100
  };
  ...
  const github = {
    ...
    async editIssueComment() { throw new Error('comment not found: the claim was superseded'); },
    async listIssueComments() { return state.comments; },
    async addPullRequestComment() {}
  };
  const run = async (call) => {
    if (call.command !== 'claude') throw new Error(`unexpected process invocation: ${call.command}`);
    ...
    return new Promise((resolve) => {
      const timer = setTimeout(() => resolve({ ok: true, code: 0, stdout: JSON.stringify({ structured_output: { ...handoffMsg(2, S2), resolved_findings: ['QA-263-015'] } }), stderr: '', error: null }), 400);
      call.signal?.addEventListener('abort', () => {
        clearTimeout(timer);
        resolve({ ok: false, code: null, stdout: '', stderr: '', error: `aborted: ${call.signal.reason?.message ?? 'lease lost'}` });
      }, { once: true });
    });
  };
  const git = {
    async resolveIssueWorktree({ prior }) { return { path: prior?.worktree_path ?? 'C:/tmp/issue-263', branch: prior?.branch ?? 'codex/agent-team-architecture', pullRequestNumber: 264 }; },
    async prepareIssueWorktree({ prior }) { return { path: prior?.worktree_path ?? 'C:/tmp/issue-263', branch: prior?.branch ?? 'codex/agent-team-architecture', pullRequestNumber: 264 }; },
  ...
  assert.equal(result.development.errors.length, 1, 'losing the lease mid-run must surface as a worker failure');
  assert.match(result.development.errors[0].error, /abort|lease/i, 'the failure must name the lost lease/abort');
  assert.equal(state.labels.has('ready-for-qa'), false, 'a run aborted by a lost lease must never reach ready-for-qa');
  assert.equal(state.labels.has('agent-working'), false, 'the aborted run must not stay agent-working');
  assert.equal(state.labels.has('ready-for-dev'), true, 'the issue must return to the claimable development state');
  assert.equal(state.labels.has('assigned-to-claude'), true, 'the issue must keep its Claude assignment for the next attempt');
});
