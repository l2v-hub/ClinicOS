...
test('production fallback behavior in frontend config is unchanged (QA-263-012 constraint)', async () => {
  ...
  assert.match(config, /PROD_BACKEND_URL = 'https:\/\/clinicos-backend-production-df88\.up\.railway\.app'/, 'the Railway production fallback must remain');
  assert.match(config, /import\.meta\.env\.PROD \? PROD_BACKEND_URL : 'http:\/\/localhost:3001'/, 'the PROD/dev fallback selection must remain');
});
