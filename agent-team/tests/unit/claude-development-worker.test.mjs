...
const workerConfig = { repository: 'l2v-hub/ClinicOS', baseBranch: 'origin/main', commandTimeoutMs: 1000, developmentTimeoutMs: 5400000, tools: TOOLS, allowedTools: ['Read', 'Edit', 'Bash(git *)'], disallowedTools: DISALLOWED, maxOutputBytes: 10000, labels: { working: 'agent-working', readyForQa: 'ready-for-qa' } };
...
test('Claude worker uses print mode, schema output, safe permissions, and the existing remediation coordinates', async () => {
  ...
  const result = await runClaudeDevelopment({ issue: { number: 263, title: 'Agent Team', body: 'contract' }, attempt: 2, config: workerConfig, github, git, run, schema: { type: 'object' }, priorQaResult: { findings: [{ finding_id: 'QA-1' }] } });
...
test('Claude worker passes the exact safe argv: restricted tool surface, allow rules, and nested-execution denies (QA-263-011)', async () => {
  const calls = [];
  const handoff = { schema_version: 1, message_type: 'development_handoff', issue_number: 263, pull_request_number: 300, attempt: 3, subject_sha: 'a'.repeat(40), acceptance_criteria: [], resolved_findings: [], commands: [], artifact_refs: [], next_actions: ['ready-for-qa'] };
  const run = async (call) => { calls.push(call); return { ok: true, code: 0, stdout: JSON.stringify({ structured_output: handoff }), stderr: '', error: null }; };
  const git = { prepareIssueWorktree: async () => ({ path: 'C:/tmp/issue-263', branch: 'codex/issue-263-agent-team', pullRequestNumber: 300 }), headSha: async () => 'a'.repeat(40) };
  const github = { addIssueComment: async () => ({ id: 1 }), addPullRequestComment: async () => {}, addLabels: async () => {}, removeLabels: async () => {} };
  await runClaudeDevelopment({ issue: { number: 263, title: 'Agent Team', body: 'contract' }, attempt: 3, config: workerConfig, github, git, run, schema: { type: 'object' }, priorQaResult: null });
  assert.deepEqual(calls[0].args, [
    '--print', '--output-format', 'json', '--permission-mode', 'acceptEdits',
    '--tools', TOOLS.join(','),
    '--allowedTools', workerConfig.allowedTools.join(','),
    '--disallowedTools', DISALLOWED.join(',')
  ]);
  assert.equal(calls[0].args.some((arg) => /bypass|dangerous/i.test(arg)), false);
});
...
test('Claude worker refuses to launch when the policy would allow nested Claude agents', async () => {
  ...
  const git = { prepareIssueWorktree: async () => { throw new Error('worktree must not be prepared'); }, headSha: async () => 'a'.repeat(40) };
  const github = {};
  await assert.rejects(
    () => runClaudeDevelopment({ issue: { number: 263, title: 't', body: 'b' }, attempt: 3, config: { ...workerConfig, tools: [...TOOLS, 'Task'] }, github, git, run, schema: { type: 'object' }, priorQaResult: null }),
    /claude development launch refused: .*nested-agent/
  );
  await assert.rejects(
    () => runClaudeDevelopment({ issue: { number: 263, title: 't', body: 'b' }, attempt: 3, config: { ...workerConfig, disallowedTools: undefined }, github, git, run, schema: { type: 'object' }, priorQaResult: null }),
