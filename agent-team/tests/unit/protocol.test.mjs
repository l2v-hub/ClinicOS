...
const schema = {
  ...
  required: ['schema_version', 'message_type', 'issue_number'],
  properties: {
    schema_version: { const: 1 },
    message_type: { enum: ['qa_result'] },
...
test('protocol rejects prose and unknown fields', () => {
  ...
  assert.throws(
    () =>
      formatProtocolComment(
        { schema_version: 1, message_type: 'qa_result', issue_number: 263, extra: true },
        schema,
      ),
    /additional property/,
  );
...
test('finding fingerprints remain stable across timestamps', () => {
  ...
  assert.equal(
    fingerprintFinding({ ...base, created_at: 'a' }),
    fingerprintFinding({ ...base, created_at: 'b' }),
  );
});
