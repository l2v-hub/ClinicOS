...
async function readLocal(config) {
  try {
    const heartbeat = JSON.parse(
      await readFile(path.join(config.runtimeRoot, 'heartbeat.json'), 'utf8'),
    );
    const age = Date.now() - Date.parse(heartbeat.at);
    return {
      supervisor_running: age < config.heartbeatTimeoutMs,
...
export async function run({ config, deps = {} }) {
  ...
  for (const label of [
    ...
    for (const listed of await github.listIssuesByLabels([label])) {
      ...
      const pullRequest = context.lastHandoff?.pull_request_number
        ? await github.viewPullRequest(context.lastHandoff.pull_request_number)
        : null;
