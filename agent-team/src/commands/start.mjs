...
export async function run({ config, repoRoot, mode, deps = {} }) {
  ...
  if (mode === 'start') {
    ...
    if (!health.developmentReady || !health.qaReady)
      return { ok: false, started: false, error: 'doctor prerequisites failed', doctor: health };
    const spawnDetached =
      deps.spawnDetached ??
      (() => {
        const child = spawn(
          process.execPath,
          [path.join(repoRoot, 'agent-team', 'src', 'cli.mjs'), 'loop'],
    ...
    while (!(await isLive())) {
      if (waited >= ackTimeoutMs)
        return {
          ...
          acknowledged: false,
          pid: child.pid,
          error: 'supervisor did not acknowledge within the timeout',
        };
      await sleep(pollMs);
      waited += pollMs;
    }
  ...
  try {
    ...
    const recovery = await recoverActiveWork({
    ...
    while (true) {
      try {
        ...
        /* keep looping until a stop is requested */
      }
      const result = await runOnce({ config, repoRoot, allowCurrentSupervisor: true });
      await writeHeartbeat(config.runtimeRoot, {
        ok: result.ok,
        result,
