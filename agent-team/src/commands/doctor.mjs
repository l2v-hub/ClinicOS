...
function requireOptions(result, options) {
  ...
  return missing.length
    ? { ok: false, detail: `missing required options: ${missing.join(', ')}` }
    : { ok: true, detail: 'ok' };
}
...
export async function runDoctor({ config, run, isSupervisorLive, probes = defaultProbes }) {
  const checks = [];
  const push = (id, blockingFor, ok, detail) =>
    checks.push({ id, ok, blockingFor, detail: ok ? 'ok' : detail });
  const invoke = (command, args) =>
    run({ command, args, cwd: process.cwd(), timeoutMs: 15000, maxOutputBytes: 65536 });
  ...
  const cli = [
    [
      ...
      'codex',
      ['--version'],
      (r) => ({ ok: r.ok, detail: r.error || r.stderr || 'check failed' }),
    ],
    [
      'codex-help',
      'qa',
    ...
    [
      ...
      'codex',
      ['exec', '--help'],
      (r) => requireOptions(r, REQUIRED_CODEX_EXEC_OPTIONS),
    ],
    [
      'codex-auth',
      'qa',
      ...
      ['login', 'status'],
      (r) => ({
        ok: r.ok && /logged in/i.test(textOf(r)),
        detail: r.error || r.stderr || 'not authenticated',
      }),
    ],
    [
      'claude-version',
    ...
    [
      ...
      'claude',
      ['--help'],
      (r) => requireOptions(r, REQUIRED_CLAUDE_OPTIONS),
    ],
    [
      'claude-auth',
      'development',
      ...
      (r) => {
        ...
        try {
          return { ok: JSON.parse(r.stdout).loggedIn === true, detail: 'not logged in' };
        } catch {
          return { ok: /logged in/i.test(textOf(r)), detail: 'not logged in' };
        }
      },
    ],
    [
    ...
    [
      ...
      (r) => {
        ...
        try {
          return {
            ok: JSON.parse(r.stdout).permissions?.push === true,
            detail: 'push permission missing for the authenticated account',
          };
        } catch {
          return { ok: false, detail: 'repository permissions unreadable' };
        }
      },
    ],
    [
      ...
      'both',
      'gh',
      ['label', 'list', '--repo', config.repository, '--limit', '200', '--json', 'name'],
      (r) => {
        ...
        try {
          ...
          return missing.length
            ? { ok: false, detail: `missing labels: ${missing.join(', ')}` }
            : { ok: true, detail: 'ok' };
        } catch {
          return { ok: false, detail: 'label list unreadable' };
        }
      },
    ],
    [
    ...
    [
      ...
      (r) => ({
        ok: r.ok && r.stdout.includes(config.repository),
        detail: r.error || r.stderr || `origin does not reference ${config.repository}`,
      }),
    ],
    [
      'git-worktree',
  ...
  for (const file of REFERENCED_FILES)
    if (!(await probes.fileExists(path.resolve(file)))) missingFiles.push(file);
  push(
    'config-files',
    'both',
    missingFiles.length === 0,
    `missing referenced files: ${missingFiles.join(', ')}`,
  );
  ...
  const timeoutsOk =
    Number.isInteger(config.developmentTimeoutMs) &&
    Number.isInteger(config.qaTimeoutMs) &&
    config.developmentTimeoutMs > config.commandTimeoutMs &&
    config.qaTimeoutMs > config.commandTimeoutMs &&
    config.developmentTimeoutMs >= 300000 &&
    config.qaTimeoutMs >= 300000;
  push(
    'timeouts-realistic',
    'both',
    timeoutsOk,
    'developmentTimeoutMs and qaTimeoutMs must exceed commandTimeoutMs and be at least 300000ms',
  );

  push(
    'runtime-root-writable',
    'both',
    await probes.writableProbe(config.runtimeRoot),
    `runtime root is not writable: ${config.runtimeRoot}`,
  );

  push(
    'duplicate-supervisor',
    'both',
    !(await isSupervisorLive({ heartbeatTimeoutMs: config.heartbeatTimeoutMs })),
    'another live supervisor owns this runtime root',
  );

  const developmentReady = checks
    .filter((check) => check.blockingFor === 'development' || check.blockingFor === 'both')
    .every((check) => check.ok);
  const qaReady = checks
    .filter((check) => check.blockingFor === 'qa' || check.blockingFor === 'both')
    .every((check) => check.ok);
