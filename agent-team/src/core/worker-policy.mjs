...
export function validateWorkerToolPolicy({ tools, allowedTools, disallowedTools } = {}) {
  ...
  if (!Array.isArray(tools) || tools.length === 0) {
    ...
    for (const tool of tools) {
      if (typeof tool !== 'string' || !BARE_TOOL.test(tool))
        errors.push(`tools entry must be a bare tool name: ${tool}`);
      else if (NESTED_AGENT_TOOLS.includes(tool))
        errors.push(`tools must not expose a nested-agent tool: ${tool}`);
    }
  }
  if (!Array.isArray(allowedTools) || allowedTools.length === 0) {
    errors.push(
      'allowedTools policy is empty or missing — configure a scoped permission policy before launching workers',
    );
  }
  if (!Array.isArray(disallowedTools) || disallowedTools.length === 0) {
    errors.push(
      'disallowedTools policy is empty or missing — nested Claude execution must be explicitly denied',
    );
  } else {
    for (const rule of disallowedTools) {
      if (typeof rule !== 'string' || !SCOPED_RULE.test(rule))
        errors.push(`disallowedTools entry must be a scoped tool rule: ${rule}`);
    }
    for (const required of REQUIRED_DISALLOWED_TOOLS) {
      if (!disallowedTools.includes(required))
        errors.push(`disallowedTools must deny nested Claude execution: missing ${required}`);
    }
  }
  for (const [key, rules] of [
    ...
    for (const rule of rules) {
      if (typeof rule === 'string' && /dangerous|bypass/i.test(rule))
        errors.push(`${key} entry must not reference bypass options: ${rule}`);
    }
  }
