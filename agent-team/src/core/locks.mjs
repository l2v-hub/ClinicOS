...
export function arbitrateClaims(claims, now = new Date()) {
  return (
    claims
      .filter((claim) => Date.parse(claim.expires_at) > now.getTime())
      .sort(
        (a, b) =>
          Date.parse(a.created_at) - Date.parse(b.created_at) || a.comment_id - b.comment_id,
      )[0] ?? null
  );
}
...
function parseClaimComments(comments, schema) {
  ...
  for (const comment of comments) {
    try {
      ...
      /* invalid or foreign comment — never a claim competitor */
    }
  }
...
export async function acquireGitHubClaim({
  github,
  config,
  schema,
  issue,
  ...
  const competitors = parseClaimComments(
...
export async function refreshGitHubClaim({
  github,
  config,
  schema,
  claim,
...
export function recoverActiveClaim({ comments, schema, issueNumber, workerId, now = new Date() }) {
  const mine = parseClaimComments(comments, schema).filter(
    (claim) => claim.issue_number === issueNumber && claim.worker_id === workerId,
  );
...
export async function acquireLocalSupervisorLock(runtimeRoot) {
  ...
  try {
    ...
    if (error.code === 'EEXIST') throw new Error('supervisor already running');
    throw error;
  }
...
export async function isSupervisorLive(
  runtimeRoot,
  { heartbeatTimeoutMs = 45000, now = () => Date.now() } = {},
) {
  try {
    const state = JSON.parse(await readFile(path.join(runtimeRoot, 'heartbeat.json'), 'utf8'));
    return now() - Date.parse(state.at) < heartbeatTimeoutMs;
  } catch {
    return false;
  }
}
...
export async function writeHeartbeat(runtimeRoot, state) {
  await mkdir(runtimeRoot, { recursive: true });
  await writeFile(
    path.join(runtimeRoot, 'heartbeat.json'),
    JSON.stringify({ ...state, pid: process.pid, at: new Date().toISOString() }),
  );
}
