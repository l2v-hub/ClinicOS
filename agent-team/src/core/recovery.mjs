...
export async function recoverActiveWork({ github, config, schemas, workerId, now = new Date() }) {
  ...
  for (const issue of working) {
    ...
    const claim = recoverActiveClaim({ comments, schema: schemas.claim, issueNumber: issue.number, workerId, now });
    if (!claim) continue;
    const refreshed = await refreshGitHubClaim({ github, config, schema: schemas.claim, claim, now });
    recovered.push({ issue_number: issue.number, lease_id: refreshed.lease_id, attempt: refreshed.attempt, expires_at: refreshed.expires_at });
  }
  return { recovered };
}
