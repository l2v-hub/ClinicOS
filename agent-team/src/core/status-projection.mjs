...
export function buildStatusProjection({ local, items, config, now = new Date() }) {
  const issues = items.map(({ issue, pullRequest }) => {
    ...
    const claims = protocol
      .filter((entry) => entry.value.message_type === 'work.claim')
      .map((entry) => ({ ...entry.value, comment_id: entry.comment_id }));
    const releasedLeases = new Set(
      protocol
        .filter((entry) => entry.value.message_type === 'work.claim_released')
        .map((entry) => entry.value.lease_id),
    );
    ...
    return {
      ...
      active_claim: activeClaim
        ? {
            lease_id: activeClaim.lease_id,
            worker_id: activeClaim.worker_id,
            attempt: activeClaim.attempt,
            expires_at: activeClaim.expires_at,
            comment_id: activeClaim.comment_id,
          }
        : null,
      ...
      last_handoff: lastHandoff
        ? {
            attempt: lastHandoff.attempt,
            subject_sha: lastHandoff.subject_sha,
            pull_request_number: lastHandoff.pull_request_number,
          }
        : null,
      last_qa: lastQa
        ? { attempt: lastQa.attempt, decision: lastQa.decision, subject_sha: lastQa.subject_sha }
        : null,
  ...
  return {
    ...
    supervisor: {
      running: local.supervisor_running,
      heartbeat_age_ms: local.heartbeat_age_ms,
      pid: local.pid ?? null,
    },
    issues,
  };
