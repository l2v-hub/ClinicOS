...
export function parseProtocolComments(comments = []) {
  ...
  for (const comment of comments) {
    ...
    } catch { /* untrusted comment body — ignore */ }
  }
  return parsed;
}

export function rebuildRemediationContext({ comments, issueNumber, config }) {
  const protocol = parseProtocolComments(comments).filter((entry) => entry.value.issue_number === issueNumber || entry.value.issue_number === undefined);
  ...
  const blocked = qaHistory.length >= (config.noProgressLimit ?? 3)
    && nextRemediationState({ qaResults: qaHistory, noProgressLimit: config.noProgressLimit ?? 3 }) === 'blocked';
  ...
  if (lastQaResult && lastQaResult.decision === 'qa-failed') {
    priorQaResult = {
      ...
      coordinates: {
        pull_request_number: lastQaResult.pull_request_number ?? lastHandoff?.pull_request_number ?? null,
