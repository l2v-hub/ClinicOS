...
export async function buildEvidenceBinding({ git, repoRoot, headSha, manifestPath, issueNumber, pullRequestNumber, correlationId, schema, now = new Date() }) {
...
export async function verifyEvidenceBinding({ git, repoRoot, envelope, pullRequestHeadSha, schema }) {
  if (schema) validateAgainstSchema(envelope, schema);
  if (envelope.subject_sha !== pullRequestHeadSha) {
    throw new Error(`evidence binding subject mismatch: envelope ${envelope.subject_sha} vs PR head ${pullRequestHeadSha}`);
  }
  const manifestRaw = await git.showFile(repoRoot, pullRequestHeadSha, envelope.manifest_path);
  if (sha256(manifestRaw) !== envelope.manifest_sha256) throw new Error(`manifest digest mismatch at PR head: ${envelope.manifest_path}`);
  const manifestBlob = await git.blobSha(repoRoot, pullRequestHeadSha, envelope.manifest_path);
  if (manifestBlob !== envelope.manifest_git_blob_sha) throw new Error(`manifest blob mismatch at PR head: ${envelope.manifest_path}`);
  const manifest = JSON.parse(manifestRaw);
  if (manifest.subject_sha !== envelope.manifest_subject_sha) throw new Error('committed manifest subject disagrees with the binding envelope');
  for (const artifact of manifest.artifacts ?? []) {
    let blob = null;
    try { blob = await git.blobSha(repoRoot, pullRequestHeadSha, artifact.path); } catch { blob = null; }
    if (blob !== artifact.git_blob_sha) throw new Error(`artifact blob mismatch at PR head: ${artifact.path}`);
    const content = await git.showFile(repoRoot, pullRequestHeadSha, artifact.path);
    if (sha256(content) !== artifact.sha256) throw new Error(`artifact digest mismatch at PR head: ${artifact.path}`);
  }
