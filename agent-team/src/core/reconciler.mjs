export async function reconcileOnce({ doctor, listDevelopment, listQa, runDevelopment, runQa }) {
  ...
  const result = {
    doctor: health,
    development: { processed: [], errors: [], skipped_reason: null },
    qa: { processed: [], errors: [], skipped_reason: null },
  };
  if (health.developmentReady) {
    for (const issue of await listDevelopment()) {
      try {
        await runDevelopment(issue);
      } catch (error) {
        result.development.errors.push({ number: issue.number, error: error.message });
      }
  ...
  if (health.qaReady) {
    for (const item of await listQa()) {
      try {
        ...
        result.qa.errors.push({ number: item.issue.number, error: error.message });
      }
