...
const loadSchema = async (repoRoot, name) =>
  JSON.parse(
    await readFile(path.join(repoRoot, 'agent-team', 'protocol', 'schemas', name), 'utf8'),
  );
...
export async function createRuntime({
  ...
  const doctor =
    ...
    (() =>
      runDoctor({
        config,
        run,
        isSupervisorLive: ({ heartbeatTimeoutMs } = {}) =>
          allowCurrentSupervisor
            ? false
            : isSupervisorLive(config.runtimeRoot, {
                heartbeatTimeoutMs: heartbeatTimeoutMs ?? config.heartbeatTimeoutMs,
              }),
      }));

  const listDevelopment = async () =>
    (
      await github.listIssuesByLabels([config.labels.readyForDev, config.labels.assignedToClaude])
    ).slice(0, config.developmentConcurrency);

  const listQa = async () => {
    const issues = (await github.listIssuesByLabels([config.labels.readyForQa])).slice(
      0,
      config.qaConcurrency,
    );
    const items = [];
    for (const issue of issues) {
      ...
      items.push({
        issue: full,
        pullRequest: await github.viewPullRequest(context.lastHandoff.pull_request_number),
        worktreePath: path.join(config.worktreeRoot, `issue-${issue.number}`),
      });
    }
    return items;
  };
  ...
  const publishBlocked = async (issue, context) => {
    const blocked = {
      ...
      qa_history: context.qaHistory
        .slice(-config.noProgressLimit)
        .map((result) => ({
    ...
    await github.removeLabels(issue.number, [
      config.labels.readyForDev,
      config.labels.assignedToClaude,
      ...(labelsOf(issue).includes(config.labels.qaFailed) ? [config.labels.qaFailed] : []),
    ]);
  ...
  const runDevelopment = async (listed) => {
    ...
    if (priorQaResult?.coordinates?.pull_request_number) {
      const pullRequest = await github.viewPullRequest(
        priorQaResult.coordinates.pull_request_number,
      );
      priorQaResult = {
        ...priorQaResult,
        coordinates: { ...priorQaResult.coordinates, branch: pullRequest.headRefName },
      };
    }
    ...
    const acquired = await acquireGitHubClaim({
      ...
      issue,
      workerId,
      role: 'claude-development',
      attempt: context.attempt,
      branch: resolved.branch ?? priorQaResult?.coordinates?.branch,
      worktree: resolved.path,
      pullRequestNumber:
        resolved.pullRequestNumber ?? priorQaResult?.coordinates?.pull_request_number,
    });
    ...
    let handoff;
    try {
      const coordinates = await git.prepareIssueWorktree({
        issue,
        prior: priorQaResult?.coordinates,
      });
      ...
      if (!isSameWorktreePath(coordinates.path, resolved.path)) {
        throw new Error(
          `prepared worktree ${coordinates.path} does not match the claimed authoritative worktree ${resolved.path}`,
        );
      }
      handoff = await runClaudeDevelopment({
        ...
        git,
        run,
        schema: schemas.development,
        priorQaResult,
        coordinates,
        lease: {
          refresh: () =>
            refreshGitHubClaim({ github, config, schema: schemas.claim, claim: acquired.claim }),
          intervalMs: config.leaseRefreshMs,
        },
      });
    } catch (error) {
      await releaseGitHubClaim({
        ...
        reason: `worker-failure: ${error.message}`.slice(0, 300),
      });
    ...
    await releaseGitHubClaim({
      ...
      schema: schemas.claim,
      claim: acquired.claim,
      reason: `development-handoff-published: ${handoff.message_id}`,
    });
    return handoff;
  };
  ...
  const refreshClaim = (claim) =>
    refreshGitHubClaim({ github, config, schema: schemas.claim, claim });
  const recoverClaim = async (issueNumber) =>
    recoverActiveClaim({
      comments: await github.listIssueComments(issueNumber),
      schema: schemas.claim,
      issueNumber,
      workerId,
    });
