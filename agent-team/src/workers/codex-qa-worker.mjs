...
export async function runCodexQa({
  issue,
  pullRequest,
  config,
  github,
  ...
  const outputPath = path.join(
    config.runtimeRoot,
    `qa-${issue.number}-${pullRequest.headRefOid}.json`,
  );
  const prompt = await readFile(
    path.join(process.cwd(), 'agent-team', 'prompts', 'codex-qa.md'),
    'utf8',
  );
  const context = {
  ...
  const result = await run({
    ...
    args: [
      ...
      worktreePath,
      '--output-schema',
      path.join(process.cwd(), 'agent-team', 'protocol', 'schemas', 'qa-result.schema.json'),
      '--output-last-message',
      outputPath,
      '-',
    ],
    ...
    timeoutMs: config.qaTimeoutMs ?? config.commandTimeoutMs,
    maxOutputBytes: config.maxOutputBytes,
  });
  if (!result.ok) throw new Error(result.error || result.stderr || 'Codex QA failed');
  const qaResult = result.finalMessage
  ...
  await verifyArtifactRefs({
    repoRoot: worktreePath,
    subjectSha: pullRequest.headRefOid,
    refs: qaResult.artifact_refs,
  });
  ...
  if (qaResult.decision === 'qa-passed')
    await github.addLabels(issue.number, [config.labels.qaPassed]);
  if (qaResult.decision === 'blocked')
