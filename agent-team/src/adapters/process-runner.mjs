...
function killTreeSync(child) {
  ...
  if (process.platform === 'win32') {
    ...
    try { spawnSync('taskkill', ['/pid', String(child.pid), '/T', '/F'], { windowsHide: true, stdio: 'ignore', timeout: 5000 }); } catch { /* fall through to child.kill() */ }
  }
...
function installExitHook() {
  ...
  process.on('exit', () => { killOwnedProcessTrees(); });
}
...
export function runProcess({ command, args = [], cwd, input, timeoutMs = 120000, maxOutputBytes = 1048576, sanitize = true, killTree = defaultKillTree, signal }) {
  const startedAt = new Date().toISOString();
  const abortReason = () => sanitizeText(`aborted: ${signal?.reason instanceof Error ? signal.reason.message : String(signal?.reason ?? 'unknown reason')}`);
  // QA-263-015: a lease already lost before launch must never start a process at all.
  if (signal?.aborted) {
    return Promise.resolve({ ok: false, code: null, error: abortReason(), command, args, pid: null, startedAt, finishedAt: new Date().toISOString(), stdout: '', stderr: '' });
  }
  ...
  return new Promise((resolve) => {
    ...
    const onAbort = () => { killTree(child); finish({ ok: false, code: null, error: abortReason() }); };
    ...
    child.on('error', (error) => { ownedChildren.delete(child); finish({ ok: false, code: null, error: sanitizeText(error.message) }); });
    child.on('close', (code) => { ownedChildren.delete(child); finish({ ok: code === 0, code, error: null }); });
    try {
