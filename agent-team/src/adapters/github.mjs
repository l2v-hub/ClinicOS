  ...
  return {
    ...
    async addIssueComment(number, body) {
      return JSON.parse(
        await invoke([
          'api',
          `repos/${config.repository}/issues/${number}/comments`,
          '-X',
          'POST',
          '-f',
          `body=${body}`,
    ...
    async listIssueComments(number) {
      return JSON.parse(
        await invoke(['api', `repos/${config.repository}/issues/${number}/comments`, '--paginate']),
      );
    },
    async viewPullRequest(number) {
      return JSON.parse(
        await invoke([
          ...
          config.repository,
          '--json',
          'number,isDraft,headRefName,headRefOid,baseRefName,state,statusCheckRollup,url',
        ]),
      );
    },
    async addPullRequestComment(number, body) {
      await invoke(['pr', 'comment', String(number), '--repo', config.repository, '--body', body]);
    },
  };
}
