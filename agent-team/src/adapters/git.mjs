...
export function createGitAdapter({ run, config }) {
  ...
  const listRegisteredWorktrees = async () => {
    ...
    for (const line of (await invoke(['worktree', 'list', '--porcelain'])).split(/\r?\n/)) {
      ...
      } else if (current && line.trim().startsWith('branch refs/heads/'))
        current.branch = line.trim().slice('branch refs/heads/'.length);
    }
    return entries;
  };
  ...
  const isValidWorktree = async (worktreePath, branch) => {
    ...
    try {
      if (
        !(await listRegisteredWorktrees()).some((entry) =>
          isSameWorktreePath(entry.path, worktreePath),
        )
      )
        return false;
      if ((await invoke(['rev-parse', '--abbrev-ref', 'HEAD'], worktreePath)) !== branch)
        return false;
      return (await invoke(['remote', 'get-url', 'origin'], worktreePath)).includes(
        config.repository,
      );
    } catch {
      return false;
  ...
  const findWorktreeForBranch = async (branch) => {
    const entry = (await listRegisteredWorktrees()).find(
  ...
  const hasLocalBranch = async (branch) => {
    try {
      await invoke(['rev-parse', '--verify', '--quiet', `refs/heads/${branch}`]);
      return true;
    } catch {
      return false;
    }
  ...
  return {
    ...
    async resolveIssueWorktree({ issue, prior }) {
      if (prior?.worktree_path && prior?.branch) {
        ...
        if (await isValidWorktree(prior.worktree_path, prior.branch))
        ...
        if (existing && existsSync(existing))
          return { path: existing, branch: prior.branch, pullRequestNumber };
        return { path: prior.worktree_path, branch: prior.branch, pullRequestNumber };
      }
      return {
        path: defaultWorktreePath(issue),
    ...
    async prepareIssueWorktree({ issue, prior }) {
      if (prior?.worktree_path && prior?.branch) {
        ...
        if (await hasLocalBranch(prior.branch))
          await invoke(['worktree', 'add', prior.worktree_path, prior.branch]);
        else
          await invoke([
            'worktree',
            'add',
    ...
    async headSha(cwd) {
      return invoke(['rev-parse', 'HEAD'], cwd);
    },
    async changedFiles(cwd) {
      return (await invoke(['diff', '--name-only', `${config.baseBranch}...HEAD`], cwd))
        .split(/\r?\n/)
        .filter(Boolean);
    },
    async blobSha(cwd, ref, filePath) {
      return invoke(['rev-parse', `${ref}:${filePath}`], cwd);
    },
