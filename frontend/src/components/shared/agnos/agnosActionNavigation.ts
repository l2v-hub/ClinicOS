...
export function writeActionNavigation(plan?: {
  ...
  if (
    plan.actionType === 'update_patient_demographics' &&
    ['phone', 'email', 'address'].includes(String(plan.fields?.field))
  )
    return { type: 'open_contacts', label: 'Contatti', patientId: plan.patientId };
...
export function readActionNavigation(text: string, answer: AssistantAnswer): AssistantNav | null {
  if (
    !/^\s*(?:apri|mostra(?:mi)?|vai|portami|fammi vedere)\b/i.test(text) ||
    answer.refusal ||
    answer.notFound
  )
    return null;
  ...
  for (const nav of answer.navigation ?? []) {
    ...
    if (
      (PATIENT_DESTINATIONS.has(nav.type) && !validId(nav.patientId)) ||
      (nav.patientId != null && !validId(nav.patientId))
    )
      return null;
...
export function navigationScope(nav: AssistantNav, role?: string) {
  if (nav.type === 'open_agenda' || nav.type === 'open_appointment')
    return {
      ...
      navKey: role === 'admin' ? 'agenda-admin' : 'agenda-operatore',
    };
  if (nav.type === 'open_beds')
    return {
      currentPatientId: undefined,
      navKey: role === 'admin' ? 'posti-letto' : 'operator-dashboard',
    };
...
export async function navigateAgnosTarget(
  nav: AssistantNav,
  handlers: {
    isAdmin: boolean;
  ...
  if (nav.type === 'open_agenda' || nav.type === 'open_appointment') {
    handlers.navigate(handlers.isAdmin ? 'agenda-admin' : 'agenda-operatore');
    return true;
  }
  if (nav.type === 'open_therapies_today') {
    handlers.navigate('terapie');
    return true;
  }
  if (nav.type === 'open_beds') {
