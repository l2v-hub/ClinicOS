...
const MSG_UNSUPPORTED = 'Riconoscimento vocale non supportato dal browser: puoi digitare il comando.';
...
export function useVoiceInput({ onFinalTranscript }: UseVoiceInputOptions) {
  ...
  const start = useCallback(async () => {
    ...
    if (!SRClass) { setError(MSG_UNSUPPORTED); return; }
    try {
    ...
    rec.onerror = (e) => {
      setError(e?.error === 'not-allowed' || e?.error === 'service-not-allowed' ? MSG_DENIED : MSG_CAPTURE);
    };
    rec.onend = () => {
      recRef.current = null;
      setListening(false);
    ...
    setInterimText('');
    setListening(true);
    try { rec.start(); } catch { recRef.current = null; setListening(false); setError(MSG_CAPTURE); }
  }, []);
