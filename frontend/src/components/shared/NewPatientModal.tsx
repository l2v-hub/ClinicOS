...
const FORM_VUOTO: NuovoPaziente = {
  ...
  codiceFiscale: '',
  phone: '', email: '', address: '', comune: '', provincia: '', cap: '',
  referenteNome: '', referenteTelefono: '', referenteRelazione: '',
  emergencyContact: '',
  provenienza: '', centroInviante: '', dataIngresso: today(),
  motivoIngresso: '', condizioniIniziali: '',
  operatoreId: '', camera: '', letto: '', statoPaziente: '', priorita: '',
  alertOperativi: '',
  notaClinicaIniziale: '', noteIniziali: '', allergie: '', farmaci: '',
  alertClinici: '', osservazioniLibere: '',
};
          ...
          <div className="npm-grid">
            ...
            <NpmField label="Cognome" required error={errors.lastName}>
              <input className={`npm-input${errors.lastName ? ' npm-input--error' : ''}`}
                value={form.lastName} onChange={f('lastName')} onBlur={blur('lastName')}
                placeholder="Rossi" autoComplete="family-name" />
            </NpmField>
            <NpmField label="Data di nascita" required error={errors.dateOfBirth}>
              <input type="date" className={`npm-input${errors.dateOfBirth ? ' npm-input--error' : ''}`}
                value={form.dateOfBirth} onChange={f('dateOfBirth')} onBlur={blur('dateOfBirth')} />
            </NpmField>
            ...
            <NpmField label="Codice fiscale" span2 hint="Maiuscolo — 16 caratteri">
              <input className="npm-input npm-mono" value={form.codiceFiscale} onChange={f('codiceFiscale')}
        ...
        <NpmCard title="Recapiti" desc="Contatti e indirizzo di residenza">
          <div className="npm-grid">
            <NpmField label="Telefono">
              <input type="tel" className="npm-input" value={form.phone} onChange={f('phone')}
                placeholder="+39 333 000 0000" autoComplete="tel" />
            </NpmField>
            <NpmField label="Email">
              <input type="email" className="npm-input" value={form.email} onChange={f('email')}
                placeholder="mario.rossi@email.it" autoComplete="email" />
            </NpmField>
            <NpmField label="Indirizzo" span2>
              <input className="npm-input" value={form.address} onChange={f('address')}
                placeholder="Via Roma 1" autoComplete="street-address" />
            </NpmField>
            <NpmField label="Comune">
              <input className="npm-input" value={form.comune} onChange={f('comune')}
                placeholder="Milano" />
            </NpmField>
            <NpmField label="Provincia">
              <input className="npm-input" value={form.provincia} onChange={f('provincia')}
                placeholder="MI" maxLength={2} style={{ textTransform: 'uppercase' }} />
            </NpmField>
            <NpmField label="CAP">
              <input className="npm-input" value={form.cap} onChange={f('cap')}
                placeholder="20100" maxLength={5} inputMode="numeric" />
            </NpmField>
          </div>
        </NpmCard>
      </>
        ...
        <NpmCard title="Provenienza e accesso" desc="Da dove arriva il paziente e quando">
          <div className="npm-grid">
            ...
            <NpmField label="Centro inviante">
              <input className="npm-input" value={form.centroInviante} onChange={f('centroInviante')}
                placeholder="es. Ospedale Niguarda, Dr. Bianchi…" />
            </NpmField>
            <NpmField label="Data ingresso">
              <input type="date" className="npm-input" value={form.dataIngresso} onChange={f('dataIngresso')} />
            </NpmField>
          </div>
        </NpmCard>

        <NpmCard title="Motivo e condizioni" desc="Diagnosi di accesso e stato clinico all'ingresso">
          <div className="npm-grid">
            <NpmField label="Motivo di ingresso" span2>
              <textarea className="npm-input npm-textarea" rows={3} value={form.motivoIngresso} onChange={f('motivoIngresso')}
                placeholder="Diagnosi principale, motivo della presa in carico…" />
            </NpmField>
            <NpmField label="Condizioni iniziali" span2>
              <textarea className="npm-input npm-textarea" rows={3} value={form.condizioniIniziali} onChange={f('condizioniIniziali')}
        ...
        <NpmCard title="Referente / Familiare" desc="Persona di riferimento e contatti d'emergenza">
          <div className="npm-grid">
            <NpmField label="Nome e cognome referente">
              <input className="npm-input" value={form.referenteNome} onChange={f('referenteNome')}
            ...
            <NpmField label="Telefono referente">
              <input type="tel" className="npm-input" value={form.referenteTelefono} onChange={f('referenteTelefono')}
                placeholder="+39 333 000 0001" />
            </NpmField>
            <NpmField label="Contatto emergenza" hint="Specificare se diverso dal referente">
              <input className="npm-input" value={form.emergencyContact} onChange={f('emergencyContact')}
          ...
          <div className="npm-grid">
            ...
            <NpmField label="Camera">
              {camere.length > 0 ? (
                ...
                <input className="npm-input" value={form.camera} onChange={f('camera')}
                  placeholder="es. 101, 12B…" />
              )}
            </NpmField>
            <NpmField label="Letto / Posto letto">
              <input className="npm-input" value={form.letto} onChange={f('letto')}
        ...
        <NpmCard title="Stato e priorità" desc="Classificazione clinica e operativa del paziente">
          <div className="npm-grid">
            ...
            <NpmField label="Priorità">
              <select className="npm-input npm-select" value={form.priorita} onChange={f('priorita')}>
                <option value="">— Seleziona —</option>
            ...
            <NpmField label="Alert operativi" span2
              hint="Informazioni urgenti per l'equipe: accessi, mobilità, comunicazione…">
              <textarea className="npm-input npm-textarea npm-textarea--alert" rows={3}
                value={form.alertOperativi} onChange={f('alertOperativi')}
      ...
      <NpmCard title="Documenti" desc="Carica documenti del paziente da file o fotocamera">
        ...
        <div className="npm-doc-upload">
          <div className="npm-doc-upload__tipo">
            ...
            <select className="npm-input npm-select" value={nuovoDocTipo}
          ...
          <div className="npm-doc-upload__btns">
            <button type="button" className="npm-doc-btn"
          ...
          <input ref={cameraInputRef} type="file" style={{ display: 'none' }}
            ...
            {docs.map(d => (
              <div key={d.id} className="npm-doc-item">
                ...
                <div className="npm-doc-item__info">
                  ...
                  <span className="npm-doc-item__meta">{d.tipo} · {d.dataCaricamento}</span>
                </div>
                <button type="button" className="icon-btn icon-btn--sm icon-btn--danger"
                  onClick={() => removeDoc(d.id)} title="Rimuovi">
                  <IcoX />
                </button>
              </div>
            ))}
