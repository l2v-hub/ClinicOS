...
interface SchemaList {
  _template: Record<string, SchemaLeaf>;
  valori?: unknown[];
  _descrizione?: string;
}
...
const isLeaf = (n: unknown): n is SchemaLeaf =>
  !!n && typeof n === 'object' && 'valore' in (n as Json) && !('_template' in (n as Json));
const isList = (n: unknown): n is SchemaList =>
  !!n && typeof n === 'object' && '_template' in (n as Json);
...
const options = (n: SchemaLeaf): string[] | null =>
  typeof n.valore === 'string' && n.valore.includes('|')
    ? n.valore
        .split('|')
        .map((s) => s.trim())
        .filter(Boolean)
...
const SLOTS: { key: string; label: string }[] = [
  { key: 'h08', label: 'Mattina' },
  { key: 'h12', label: 'Pranzo' },
  { key: 'h16', label: 'Pomerig.' },
  { key: 'h20', label: 'Sera' },
  { key: 'h22', label: 'Notte' },
];
...
export function ImportReviewFull({
  schema,
  full,
  rawText,
  documents,
    ...
    return (
      <div className={`irf-field${longText ? ' irf-field--wide' : ''}`} key={path.join('.')}>
        ...
        {isBool ? (
          ...
          <textarea
            className="irf-input irf-textarea"
            value={String(raw ?? '')}
            onChange={(e) => update(path, e.target.value)}
            rows={2}
          />
  ...
  function renderGenericList(node: SchemaList, key: string, path: (string | number)[]) {
    ...
    const blank = () =>
      Object.fromEntries(
        tmplKeys.map((k) => [k, typeof node._template[k].valore === 'boolean' ? false : '']),
      );
    return (
      <details className="irf-group" key={path.join('.')}>
        <summary>
          {node._descrizione?.split('.')[0] || key} ({items.length})
        </summary>
        <button
          type="button"
          className="irf-add"
  ...
  function addRow(list: string, blank: Json) {
    update(['cartella', list], [...cart(list), blank]);
  }
  function delRow(list: string, idx: number) {
    update(
      ['cartella', list],
      cart(list).filter((_, i) => i !== idx),
    );
  }
  function tmplOptions(list: string, field: string): string[] {
    const leaf = ((cartSchema[list] as SchemaList)?._template ?? {})[field] as
      SchemaLeaf | undefined;
  ...
  function cellArea(list: string, idx: number, field: string, placeholder = '') {
    return (
      <textarea
        className="irf-cell irf-textarea"
        rows={2}
        placeholder={placeholder}
        value={String((cart(list)[idx]?.[field] as string) ?? '')}
        onChange={(e) => update(['cartella', list, idx, field], e.target.value)}
  ...
  function renderDiario() {
    const tmpl = ((cartSchema.diarioMedico as SchemaList)?._template ?? {}) as Record<
      string,
      SchemaLeaf
    >;
    ...
    const rl = (k: string, i: number) =>
      tmpl[k] ? renderLeaf(tmpl[k], k, ['cartella', 'diarioMedico', i, k]) : null;
    return (
      <section className="irf-sec" key="diarioMedico">
        ...
        {items.map((_, i) => (
          <div className="irf-diary" key={i}>
            ...
            {rl('testo', i)}
            <div className="irf-grid irf-grid--3">
              {rl('operatore', i)}
              {rl('prescrizione', i)}
              {rl('evoluzione', i)}
            </div>
    ...
    return (
      <section className="irf-sec" key={groupKey}>
        ...
        <div className={`irf-grid irf-grid--${cols}`}>
          {keys.map((k) =>
            renderNode((group as Json)[k] as SchemaNode, k, ['cartella', groupKey, k]),
          )}
        </div>
      </section>
    );
  ...
  const rawShown = useMemo(() => {
    ...
    return t
      .split('\n')
      .filter((l) => l.toLowerCase().includes(rawQuery.toLowerCase()))
      .join('\n');
  }, [rawText, rawQuery]);

  const anagKeys = Object.keys(anagSchema).filter((k) => !k.startsWith('_'));
  const otherCartKeys = Object.keys(cartSchema).filter(
    (k) => !k.startsWith('_') && !SPECIAL.has(k),
  );

  return (
    <div className="irf2">
      <p className="irf-hint">
        Controlla e integra <strong>tutti</strong> i dati estratti dal documento. Niente viene
        salvato finché non premi “Crea paziente”.
      </p>
      ...
      <section className="irf-sec">
        ...
        <table className="irf-table">
          ...
          <tbody>
            {diagnosi.map((_, i) => (
              <tr key={i}>
                <td>{cellInput('diagnosi', i, 'codiceICD', 'ICD')}</td>
                <td>{cellInput('diagnosi', i, 'descrizione', 'Descrizione')}</td>
                <td>{cellInput('diagnosi', i, 'tipo')}</td>
                <td>{cellInput('diagnosi', i, 'stato')}</td>
                <td>
                  <button
                    type="button"
                    className="irf-del"
            ...
            {diagnosi.length === 0 && (
              <tr>
                <td colSpan={5} className="irf-empty">
                  Nessuna diagnosi
                </td>
              </tr>
            )}
      ...
      <section className="irf-sec">
        ...
        <table className="irf-table">
          <thead>
            <tr>
              ...
              <th>Via</th>
              {SLOTS.map((s) => (
                <th key={s.key} className="irf-slot">
                  {s.label}
                </th>
              ))}
              <th></th>
          ...
          <tbody>
            {farmaci.map((row, i) => (
              <tr key={i}>
                <td>{cellInput('farmaci', i, 'nome', 'Nome')}</td>
                <td>{cellInput('farmaci', i, 'dose', 'Dose')}</td>
                <td>{cellInput('farmaci', i, 'via')}</td>
                {SLOTS.map((s) => (
                  <td key={s.key} className="irf-slot">
                    <input
                      type="checkbox"
                      checked={!!row[s.key] && row[s.key] !== ''}
                      onChange={(e) =>
                        update(['cartella', 'farmaci', i, s.key], e.target.checked ? '✓' : '')
                      }
                    />
                  </td>
                ))}
            ...
            {farmaci.length === 0 && (
              <tr>
                <td colSpan={SLOTS.length + 4} className="irf-empty">
                  Nessun farmaco
                </td>
              </tr>
            )}
        ...
        <p className="irf-note">
          Le fasce orarie (Mattina/Pranzo/Pomeriggio/Sera/Notte) diventano la terapia
          somministrabile del paziente.
        </p>
      </section>
      ...
      <section className="irf-sec">
        ...
        <table className="irf-table">
          ...
          <tbody>
            {parametri.map((row, i) => (
              <tr key={i} className={`irf-vit irf-vit--${String(row.stato || 'normale')}`}>
                ...
                <td>
                  <select
                    className="irf-cell"
                    value={String(row.stato || 'normale')}
                    onChange={(e) =>
                      update(['cartella', 'parametriVitali', i, 'stato'], e.target.value)
                    }
                  >
                    {VITALE_STATI.map((s) => (
      ...
      <section className="irf-sec">
        ...
        <table className="irf-table">
          ...
          <tbody>
            {terapie.map((_, i) => (
              <tr key={i}>
                <td>{cellSelect('terapie', i, 'tipo')}</td>
                <td>{cellInput('terapie', i, 'descrizione', 'Descrizione/schema')}</td>
            ...
            {terapie.length === 0 && (
              <tr>
                <td colSpan={4} className="irf-empty">
                  Nessuna terapia
                </td>
              </tr>
            )}
      ...
      <section className="irf-sec">
        ...
        <table className="irf-table">
          ...
          <tbody>
            {rischi.map((row, i) => (
              <tr key={i} className={`irf-lvl irf-lvl--${String(row.livello || '')}`}>
                <td>{cellSelect('indicatoriRischio', i, 'tipo')}</td>
                <td>{cellSelect('indicatoriRischio', i, 'livello')}</td>
      ...
      <section className="irf-sec">
        ...
        <div className="irf-grid">
          {otherCartKeys.map((k) => renderNode(cartSchema[k] as SchemaNode, k, ['cartella', k]))}
        </div>
      </section>
      ...
      <section className="irf-sec">
        <h3>Documenti ({documents.length})</h3>
