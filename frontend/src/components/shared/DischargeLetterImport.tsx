  ...
  const processImage = useCallback(
    async (srcFile: File) => {
      ...
      try {
        ...
        const {
          data: { text },
        } = await worker.recognize(srcFile);
    ...
    <div className="dli-overlay" onClick={onClose}>
      <div className="dli-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="dli-header">
          ...
          <button className="dli-close" onClick={onClose} aria-label="Chiudi">
            &times;
          </button>
        </div>
            ...
            {STEP_LABELS.map((s) => {
              ...
              return (
                <div key={s.key} className={`dli-step ${cls}`}>
                  <span className="dli-step__num">{sIdx < stepIdx ? '\u2713' : s.num}</span>
                  {s.label}
                </div>
              );
            })}
        ...
        <div className="dli-body">
          ...
          {step === 'review' && (
            <>
              ...
              {showOcr && (
                <div className="dli-ocr-text">{ocrText || 'Nessun testo riconosciuto'}</div>
              )}
            </>
          )}
          ...
          {step === 'applied' && (
            <div className="dli-progress" style={{ padding: '40px 0' }}>
              ...
              <div
                className="dli-progress__text"
                style={{ marginTop: 12, fontWeight: 600, color: '#16A37B' }}
              >
