...
export function buildSegments(
  ...
  for (const a of valid) {
    ...
    if (a.startOffset > cursor) out.push(...splitIllegible(rawText.slice(cursor, a.startOffset), { bold: false }));
    out.push({ text: a.text, bold: true, illegible: false, tag: a.tag, className: styles[a.tag]?.className });
    cursor = a.endOffset;
  }
