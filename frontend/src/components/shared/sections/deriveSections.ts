...
export function assertNoLegacyImportArrays(
  ...
  for (const k of LEGACY_IMPORT_ARRAYS) {
    if (Array.isArray(draft[k]))
      throw new Error(
        `LEGACY_IMPORT_CONTRACT_DETECTED: ${k}[] is not allowed in the discharge import draft`,
      );
  }
}
...
export function sectionsFromNarrative(draft: NarrativeDraft): SectionsResult {
  ...
  const annFor = (italian: string): SemanticAnnotation[] =>
    tags
      .filter((t) => t.sectionKey === italian)
      .map((t) => ({
        tag: t.tagType as SemanticAnnotation['tag'],
        text: t.text,
  ...
  for (const f of FIELDS) {
    ...
    sections.push({
      sectionKey: f.key,
      rawText: text,
      annotations: annFor(f.italian),
      sourceRanges: srcFor(f.italian),
    });
  }
  ...
  if ((draft.allergiesText ?? '').trim()) {
    sections.unshift({
      ...
      rawText: draft.allergiesText ?? '',
      annotations: annFor('ALLERGIE'),
      sourceRanges: srcFor('ALLERGIE'),
    });
  }
  return {
    sections,
    allergies: {
      status: draft.allergyStatus ?? 'not_documented',
      rawText: draft.allergiesText ?? '',
    },
    demographics: {
      ...
      sex: draft.sex ?? '',
      codiceFiscale: draft.fiscalCode ?? '',
