...
export function DiagnosisEditor({ value, onChange, readOnly, operatoreNome }: SectionProps<Diagnosi[]>) {
  const list = value ?? [];
  ...
  function saveDiagnosi(next: Diagnosi[]) { onChange(next); }

  function addDiagnosi() {
    if (!diagForm.descrizione) return;
    saveDiagnosi([
      { id: uid(), descrizione: '', tipo: 'principale', stato: 'attiva', dataInsorgenza: todayStr(), operatore: operatoreNome ?? '', note: '', createdAt: nowISO(), ...diagForm } as Diagnosi,
      ...list,
    ]);
  ...
  function deleteDiagnosi(id: string) { saveDiagnosi(list.filter(d => d.id !== id)); }
  ...
  return (
    <ClinicalTableSection
        ...
        count={list.length}
        countLabel="diagnosi"
        actions={!readOnly ? <button className="btn-sm" onClick={() => { setDiagForm({}); setShowAddDiag(true); }}>+ Aggiungi</button> : undefined}
      >
