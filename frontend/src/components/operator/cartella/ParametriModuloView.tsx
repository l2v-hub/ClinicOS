...
function matchCol(colKey: string, etichetta: string): boolean {
  ...
  switch (colKey) {
    case 'PA1':
      return (
        (e.includes('press') || e.includes(' pa') || e.startsWith('pa')) &&
        !e.includes('2') &&
        !e.includes('second')
      );
    case 'PA2':
      return (e.includes('press') || e.includes('pa')) && (e.includes('2') || e.includes('second'));
    case 'FC':
      return e.includes('card') || e.includes('fc') || e.includes('freq');
    case 'SPO2':
      return e.includes('spo') || e.includes('sat') || e.includes('ossig');
    case 'TEMP':
      return e.includes('temp') || e.includes('febbre');
    case 'DTX8':
      return (e.includes('glic') || e.includes('dtx') || e.includes('gluc')) && e.includes('08');
    case 'DTX12':
    ...
    case 'CATET':
      return e.includes('catet') || e.includes('vescic');
    default:
      return false;
  }
}
  ...
  return (
    <div className="fm fm--landscape parametri-modulo-wrap">
      ...
      >
        <div>
          ...
          <div style={{ fontSize: '10pt', display: 'flex', gap: 24 }}>
            <span>
              Cognome: <strong style={{ fontSize: '11pt' }}>{paziente.lastName}</strong>
            </span>
            <span>
              Nome: <strong style={{ fontSize: '11pt' }}>{paziente.firstName}</strong>
            </span>
          </div>
        </div>
        <div style={{ textAlign: 'right', fontSize: '10pt' }}>
          <div>
            Mese / Anno:{' '}
            <strong style={{ textTransform: 'capitalize', fontSize: '11pt' }}>{mese}</strong>
          </div>
        </div>
      </div>
            ...
            {COLS.map((c) => (
              <th key={c.key} style={{ fontSize: '7.5pt' }}>
                ...
                {c.sub && (
                  <>
                    <br />
                    <span style={{ fontWeight: 400, fontSize: '7pt' }}>{c.sub}</span>
                  </>
                )}
              </th>
            ))}
      ...
      <div style={{ marginTop: 12, display: 'flex', gap: 40, fontSize: '8.5pt' }}>
        <div>
          Firma responsabile turno M:{' '}
          <span
            style={{ display: 'inline-block', width: 120, borderBottom: '1px solid #333' }}
          ></span>
        </div>
        <div>
          Firma responsabile turno P:{' '}
          <span
            style={{ display: 'inline-block', width: 120, borderBottom: '1px solid #333' }}
          ></span>
        </div>
