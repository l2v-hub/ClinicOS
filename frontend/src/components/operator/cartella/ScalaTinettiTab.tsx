...
const EQUILIBRIO_ITEMS: {
  ...
  },
  {
    ...
    options: [
      { v: 0, label: '0 — Passi discontinui' },
      { v: 1, label: '1 — Passi continui' },
    ],
  },
  {
    ...
    options: [
      { v: 0, label: '0 — Instabile (si aggrappa)' },
      { v: 1, label: '1 — Stabile' },
    ],
  },
  {
...
const ANDATURA_ITEMS: {
...
}[] = [
  {
    ...
    options: [
      { v: 0, label: '0 — Esitazione / passi multipli' },
      { v: 1, label: '1 — Nessuna esitazione' },
    ],
  },
  {
    ...
    options: [
      { v: 0, label: '0 — Piede dx non supera il sx' },
      { v: 1, label: '1 — Piede dx supera il sx' },
    ],
  },
  {
    key: 'altezzaPassoDx',
    ...
    options: [
      { v: 0, label: '0 — Piede dx striscia' },
      { v: 1, label: '1 — Piede dx si solleva' },
    ],
  },
  {
    ...
    options: [
      { v: 0, label: '0 — Piede sx non supera il dx' },
      { v: 1, label: '1 — Piede sx supera il dx' },
    ],
  },
  {
    key: 'altezzaPassoSx',
    ...
    options: [
      { v: 0, label: '0 — Piede sx striscia' },
      { v: 1, label: '1 — Piede sx si solleva' },
    ],
  },
  {
    key: 'simmetria',
    ...
    options: [
      { v: 0, label: '0 — Asimmetrico' },
      { v: 1, label: '1 — Simmetrico' },
    ],
  },
  {
    key: 'continuita',
    ...
    options: [
      { v: 0, label: '0 — Interruzioni / fermate' },
      { v: 1, label: '1 — Continuo' },
    ],
  },
  {
    key: 'traiettoria',
  ...
  },
  {
    ...
    options: [
      { v: 0, label: '0 — Talloni distanti' },
      { v: 1, label: '1 — Talloni vicini' },
    ],
  },
];
        ...
        {options.map((o) => (
          <label key={o.v} className={`braden-option${value === o.v ? ' selected' : ''}`}>
            ...
            <div className="braden-option__content">
              ...
              <div>
                <div className="braden-option__title">
  ...
  return (
    <div className="fm braden-modulo">
      ...
      <table className="braden-modulo-table">
        <thead>
          <tr>
            <th className="col-indicator" style={{ fontWeight: 700 }}>
              EQUILIBRIO
            </th>
          ...
          {EQUILIBRIO_ITEMS.map((item) => {
            ...
            return (
              <tr key={item.key}>
                ...
                <td className="score-cell selected" style={{ textAlign: 'left', paddingLeft: 8 }}>
                  {val >= 0 ? `${val} — ${optLabel.split('—')[1]?.trim() ?? ''}` : '—'}
                </td>
              </tr>
            );
          })}
          <tr className="totale-row">
            <td style={{ textAlign: 'right', padding: '4px 10px', fontWeight: 700 }}>
              Totale Equilibrio
            </td>
...
function TinettiHistoryTable({
  ...
  type Row = ScalaTinettiValutazione & {
    _balance: number;
    _gait: number;
    _total: number;
    _rischio: string;
    _rischioClass: string;
  };
  ...
  const columns: ColumnDef<Row>[] = [
    ...
    },
    {
      ...
      sortable: true,
      render: (_v, row) => (
        <span style={{ textAlign: 'center', display: 'block' }}>{row._balance} / 16</span>
      ),
    },
    {
      key: '_gait',
      ...
      sortable: true,
      render: (_v, row) => (
        <span style={{ textAlign: 'center', display: 'block' }}>{row._gait} / 12</span>
      ),
    },
    {
      key: '_total',
      ...
      sortable: true,
      render: (_v, row) => (
        <span style={{ fontWeight: 700, display: 'block', textAlign: 'center' }}>
          {row._total} / 28
        </span>
      ),
    },
...
export function ScalaTinettiTab({ cartella, paziente, onUpdate, operatoreNome }: Props) {
  ...
  function set(f: Partial<TinettiFormState>) {
    setForm((p) => ({ ...p, ...f }));
  }
  ...
  const anyFilled =
    ALL_BALANCE_KEYS.some((k) => (form[k] as number) >= 0) ||
    ALL_GAIT_KEYS.some((k) => (form[k] as number) >= 0);
            ...
            <div className="cr-inline-form braden-form">
              ...
              {anyFilled && (
                <div className="braden-score-summary">
                  <span>
                    Equilibrio: <strong>{balanceScore} / 16</strong>
                  </span>
                  <span>
                    Andatura: <strong>{gaitScore} / 12</strong>
                  </span>
                  <span>
                    Totale: <strong>{totalScore} / 28</strong>
                  </span>
                  <span className={`badge ${r.cls}`}>{r.label}</span>
                </div>
              )}
