    ...
    <div className="fm contenzione-modulo">
      ...
      <div className="fm-inline" style={{ marginBottom: 6 }}>
        <div className="fm-inline__item">
          ...
          <span className="fm-inline__val">{c ? `${c.dataInizio.split('-').reverse().join('/')} ${c.oraInizio}` : ''}</span>
        </div>
        <div className="fm-inline__item">
      ...
      <div className="fm-inline" style={{ marginBottom: 10 }}>
        <div className="fm-inline__item">
          ...
          <span className="fm-inline__val">{c?.dataFine ? `${c.dataFine.split('-').reverse().join('/')} ${c.oraFine}` : ''}</span>
        </div>
        <div className="fm-inline__item">
          <span className="fm-inline__lbl">firma medico</span>
      ...
      <div style={{ margin: '10px 0', border: '1px solid #999', padding: '8px 10px' }}>
        <div style={{ fontWeight: 700, fontSize: '9pt', textDecoration: 'underline', marginBottom: 8 }}>Motivazioni:</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 20px' }}>
          {motivazioni.map(m => <Cb key={m.key} checked={m.checked} label={m.label} />)}
        </div>
      </div>
      ...
      {/* Declaration */}
      <div className="declaration-text" style={{ fontSize: '8.5pt', lineHeight: 1.7, border: '1px solid #ccc', padding: 10, marginTop: 12, background: '#fafafa' }}>
        Il sottoscritto <span className="fm-fill-line" style={{ minWidth: 180, display: 'inline-block' }}></span> dichiara di essere stato informato e di aver
      ...
      <div style={{ marginTop: 16, fontSize: '8.5pt', fontStyle: 'italic', borderTop: '1px solid #ccc', paddingTop: 10 }}>
        Io sottoscritto medico proponente dichiaro di aver informato il paziente e il parente sulla necessità e sulle conseguenze di tale pratica.
      </div>
...
export function ContenzioniTab({ cartella, paziente, onUpdate, operatoreNome }: Props) {
  ...
  function terminaContenzione(id: string) {
    onUpdate({ contenzioni: list.map(c => c.id === id ? { ...c, dataFine: todayStr(), oraFine: nowTime(), attiva: false } : c) });
  }

  const moduloData = moduloTarget ? (list.find(c => c.id === moduloTarget) ?? null) : (list[0] ?? null);
          ...
          <div className="cr-inline-form">
            ...
            <div className="form-row-3col">
              <div className="form-row">
                ...
                <input type="date" className="form-input" value={form.dataInizio} onChange={e => set({ dataInizio: e.target.value })} />
              </div>
              <div className="form-row">
            ...
            <div className="form-row-2col">
              <div className="form-row" style={{ paddingTop: 8 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', marginBottom: 8 }}>
                  ...
                  <span className="form-label" style={{ margin: 0 }}>Autorizzata dal medico</span>
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
                  <input type="checkbox" checked={form.autorizzazioneTutore} onChange={e => set({ autorizzazioneTutore: e.target.checked })} />
                  <span className="form-label" style={{ margin: 0 }}>Autorizzata da tutore/familiare</span>
                </label>
              </div>
            </div>
          ...
          <>
            {attive.length > 0 && (
              <div style={{ marginBottom: 24 }}>
                <div className="cr-section-title" style={{ marginBottom: 12 }}>Attive ({attive.length})</div>
                {attive.map(c => <ContenzioneCard key={c.id} c={c} onEdit={() => startEdit(c)} onDelete={() => onUpdate({ contenzioni: list.filter(x => x.id !== c.id) })} onTermina={() => terminaContenzione(c.id)} onModulo={() => { setModuloTarget(c.id); setModulo(true); }} />)}
              </div>
            )}
            {storiche.length > 0 && (
              <div>
                <div className="cr-section-title" style={{ marginBottom: 12, color: 'var(--text-muted)' }}>Storico ({storiche.length})</div>
                {storiche.map(c => <ContenzioneCard key={c.id} c={c} onEdit={() => startEdit(c)} onDelete={() => onUpdate({ contenzioni: list.filter(x => x.id !== c.id) })} onModulo={() => { setModuloTarget(c.id); setModulo(true); }} />)}
              </div>
            )}
          </>
        )}
...
function ContenzioneCard({ c, onEdit, onDelete, onTermina, onModulo }: {
  c: Contenzione; onEdit: () => void; onDelete: () => void; onTermina?: () => void; onModulo: () => void;
}) {
  return (
    <div className={`contenzione-card${c.attiva ? ' contenzione-card--attiva' : ''}`}>
      <div className="contenzione-card__header">
        <span className={`badge ${c.attiva ? 'badge--red' : 'badge--gray'}`}>{c.attiva ? 'ATTIVA' : 'CONCLUSA'}</span>
        <span className="badge badge--blue">{TIPO_LABEL[c.tipo] ?? c.tipo}</span>
        <span className="cr-meta">Inizio: {fmtDate(c.dataInizio)} {c.oraInizio}</span>
        {c.dataFine && <span className="cr-meta">Fine: {fmtDate(c.dataFine)} {c.oraFine}</span>}
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
          <button className="icon-btn icon-btn--sm" onClick={onModulo} title="Vista modulo">📋</button>
          {onTermina && c.attiva && <button className="btn-secondary btn-sm" onClick={onTermina}>Termina</button>}
          ...
          <button className="icon-btn icon-btn--sm icon-btn--danger" onClick={onDelete} title="Elimina">
