...
export type MnaInputPath =
  `measurements.${MnaMeasurementKey}` | `measurementDates.${MnaMeasurementKey}`;
/** Session-owned editing metadata, never serialized to the assessment API. */
export interface MnaLocalInputs {
  raw: Partial<Record<MnaInputPath, string>>;
  errors: Partial<Record<MnaInputPath, string>>;
}
