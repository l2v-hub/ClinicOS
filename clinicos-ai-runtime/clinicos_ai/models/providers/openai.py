...
def build(spec: ModelSpec, role: str, temperature: float, timeout_seconds: int) -> BuiltModel:  # noqa: ARG001
    def build_agent():
        ...
        class CompletionOpenAI(CompletionMetadataMixin, OpenAIChat):
            pass

        return Agent(model=CompletionOpenAI(id=spec.model_id, temperature=temperature), markdown=False, telemetry=False)
